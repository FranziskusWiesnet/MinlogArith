;; 2025-03-23.  examplesarithgcd_pos.scm.  Based on Franziskus Wiesnet's
 ...
 ;; Case p1 = S_1...
  ...
  (simp "IfFalse")
   ...
   ;; Inductionhypothesis Case 3
    ...
    (simp "PosTimesPlusDistrLeft")
    (simp "PosPlusAssoc")
    (simphyp 11 (pf "PosGcd(SOne p0)(SOne p1--SOne p0)=PosGcd(SOne p0)(p1--p0)"))
    (simp 12)
    (ng)
   ...
   ;; Inductionhypothesis Case 4
    ...
    (elim)
    (assume "p3" 10)
    (simphyp 10 (pf "PosGcd(SOne p0)(SZero p1--SZero p0)=PosGcd(SOne p0)(p1--p0)"))
    (drop 5 6 7 8 9 10)
    (intro 1)
    (intro 1)
    (intro 1)
...
;;  ​We define the GCD using the Euclidean algorithm to compare it with Stein's algorithm.
...
(use-with "PosMonMaxProp" (pt "[q0]q0*q<=p") (pt "Succ(PosLog p -- PosLog q)") "?")
(use 1)
;;(cp)
(save "RoundDivProp0")
...
(use "PosNotLeToLt")
(assume 1)
(inst-with "PosMonMaxNegProp" (pt "[q0]q0*q<=p") (pt "Succ(PosLog p -- PosLog q)") "?" "?" (pt "RoundDiv p q + 1") "?")
(bpe-ng)
...
;; Case: there is a proper reminder by dividing p1 by p0
 ...
 ;;Consindering the four cases of the induction hypothesis
 (elim 9)
 ;;Case 1
  (elim)
  (assume "q" 11)
  ...
  (simp (pf "PosGcd p0(p1--RoundDiv p1 p0*p0) = PosGcd (q*(p1--RoundDiv p1 p0*p0))(p1--RoundDiv p1 p0*p0)"))
  (use "PosGcdTimesId1")
  (simp 11)
  (use "Truth")
  (assume 12)
