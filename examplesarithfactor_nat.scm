...
;;Case Odd n
 ...
 ;;Case n=3 -> F
  ...
  ;;Case n=5->F
   (assume 5)
   ;;Abbreviation l
   (assert "exi l(l=NatLeastUp (cNatSqr n) (NatHalf (Pred n)) ([l]NatSquare (l*l--n)))")
   ...
   ;;Case l=NatHalf(Pred n)
    ...
    (simp 7)
    (use-with "NatLeastUpLtElim" (pt "cNatSqr n") (pt "NatHalf(Pred n)") (pt "[l0]NatSquare(l0*l0--n)") "?" "?")
    ...
    (intro 0 (pt "l+m"))
    (intro 0 (pt "l--m"))
    ;;The frist two parts of the conjunction follow by the last one, hence we prove it first:
    (assert "n=(l+m)*(l--m)")
     (simp "<-" "NatThirdBinom")
    ...
    (assert "1<l+m")
     ...
     (use "NatSuccLeToLt")
     (inst-with "NatLeastUpLBound" (pt "cNatSqr n") (pt "NatHalf(Pred n)") (pt "[l0]NatSquare(l0*l0--n)") "?")
     ...
     (assert "n<n")
      ...
      (use "NatLeTrans" (pt "NatLeast(Succ n)([m]n<=m*m) * NatLeast(Succ n)([m]n<=m*m)"))
      (use 15)
      (use "NatLeMonSquare")
      (use "NatLtSuccToLe")
      (use 14)
    ...
    (split)
     (use 13)
    ;;Secound part of the conjunction
    (split)
    (cases (pt "l--m"))
    ;;Cases l--m = 0
     (assume 14)
...
;;  "/home/fwiesnet/Minlog/minlog/fermat_nat.hs"
