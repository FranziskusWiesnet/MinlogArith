...
(define (type-to-subtypes type)
 (adjoin type (type-to-proper-subtypes type)))

(define (type-to-proper-subtypes type)
  (case (tag type)
    ((tvar tconst) '())
    ((alg) (apply union (map type-to-subtypes (alg-form-to-types type))))
    ((arrow) (union (type-to-subtypes (arrow-form-to-arg-type type))
		    (type-to-subtypes (arrow-form-to-val-type type))))
    ((star) (union (type-to-subtypes (star-form-to-left-type type))
		   (type-to-subtypes (star-form-to-right-type type))))
    (else '())))

;; (type-to-subtypes (py "boole yprod unit"))
;; ((alg "yprod" (alg "boole") (alg "unit"))
;;   (alg "boole")
;;   (alg "unit"))

(define (type-recursive? type)
  (and
   (arrow-form? type)
   (let ((arg-types (arrow-form-to-arg-types type))
	 (final-val-type (arrow-form-to-final-val-type type)))
     (member final-val-type (apply union (map type-to-subtypes arg-types))))))

(define (alg-to-total-inhab alg)
  (let* ((alg-name (alg-form-to-name alg))
	 (types (alg-form-to-types alg))
	 (tvars (alg-name-to-tvars alg-name))
	 ...
	 (constr-types (map cadr typed-constr-names))
	 (constrs (map (lambda (s) (constr-name-and-tsubst-to-constr
					   s tsubst))
			      constr-names))
	 (constrs-without-rec-arg-types
	  (list-transform-positive constrs
	    (lambda (c) (not (type-recursive? (const-to-type c))))))
	 (constrs-without-rec-arg-types-and-totally-inhab-arg-types
	  (list-transform-positive constrs-without-rec-arg-types
	    (lambda (c)
	      (let* ((arg-types (arrow-form-to-arg-types (const-to-type c)))
		     (inhabs (map type-to-total-inhab arg-types)))
		(apply and-op inhabs))))))
    (if
     (null? constrs-without-rec-arg-types-and-totally-inhab-arg-types)
     #f
     (let* ((constr
	     (car constrs-without-rec-arg-types-and-totally-inhab-arg-types))
	    (type (const-to-type constr))
	    (arg-types (arrow-form-to-arg-types type))
	    (args (map type-to-total-inhab arg-types))
	    (constr-term (make-term-in-const-form constr)))
       (apply mk-term-in-app-form (cons constr-term args))))))

(define (type-to-total-inhab type)
  (case (tag type)
    ((tvar tconst) #f)
    ((alg) (alg-to-total-inhab type))
    ((arrow)
     (let* ((arg-type (arrow-form-to-arg-type type))
	    (val-type (arrow-form-to-val-type type))
	    (inhab (type-to-total-inhab val-type))
	    (var (type-to-new-var arg-type)))
       (make-term-in-abst-form var inhab)))
    ((star)
     (let* ((left (star-form-to-left-type type))
	    (right (star-form-to-right-type type))
	    (inhableft (type-to-total-inhab left))
	    (inhabright (type-to-total-inhab right)))
       (make-term-in-pair-form inhableft inhabright)))
    (else (myerror "type-to-total-inhab" "type expected" type))))

;; End additions 2025-10-07

;; Added 2026-01-23

;; For a type alpha_0 ysum alpha_1 ysum ... ysum alpha_n and i<=n we
;; define a term (injterm types index) denoting the injection of an
;; object of type alpha_i into the sum type.  This uses functions
;; (make-inl-term type0 type1) and (make-inr-term type0 type1)

(define (make-inl-term type0 type1)
  (if (not (type? type0)) (myerror "make-inl-term" "type expected" type0))
  (if (not (type? type1)) (myerror "make-inl-term" "type expected" type1))
  (let* ((const (constr-name-to-constr "InL"))
	 (tvars (const-to-tvars const)) ;alpha1, alpha2
	 (tsubst (make-substitution tvars (list type0 type1)))
	 (subst-const (const-substitute const tsubst #f)))
    (make-term-in-const-form subst-const)))

;; (pp (make-inl-term (py "alpha") (py "beta")))
;; (InL alpha beta)
;; (pp (term-to-type (make-inl-term (py "alpha") (py "beta"))))
;; alpha=>alpha ysum beta

(define (make-inr-term type0 type1)
  (if (not (type? type0)) (myerror "make-inr-term" "type expected" type0))
  (if (not (type? type1)) (myerror "make-inr-term" "type expected" type1))
  (let* ((const (constr-name-to-constr "InR"))
	 (tvars (const-to-tvars const)) ;alpha2, alpha1
	 (tsubst (make-substitution tvars (list type0 type1)))
	 (subst-const (const-substitute const tsubst #f)))
    (make-term-in-const-form subst-const)))

;; (pp (make-inr-term (py "beta") (py "alpha")))
;; (InR beta alpha)
;; (pp (term-to-type (make-inr-term (py "beta") (py "alpha"))))
;; beta=>alpha ysum beta

(define (injterm types index)
  (if (< (length types) index)
      (myerror "injterm" index "should be less than" (length types)))
  (let ((var0 (make-var (car types) -1 1 "")))
    (if (= 1 (length types))
	(make-term-in-abst-form var0 (make-term-in-var-form var0))
	(if (= 0 index)
	    (let* ((type (car types))
		   (sumtype (apply mk-ysum (cdr types))))
	      (make-inl-term type sumtype))	      
	    (let* ((type (list-ref types index))
		   (var (make-var type -1 1 ""))
		   (prev (injterm (cdr types) (- index 1)))
		   (appterm (make-term-in-app-form
			     prev (make-term-in-var-form var)))
		   (sumtype (apply mk-ysum (cdr types)))
		   (inrterm (make-inr-term sumtype (car types))))
	      (make-term-in-abst-form
	       var (make-term-in-app-form inrterm appterm)))))))

;; (pp (injterm (list (py "alpha0") (py "alpha1") (py "alpha2")) 0))
;; (InL alpha0 (alpha1 ysum alpha2))

;; End additions 2026-01-23
...
(define (totally-inhabited? type)
  (cond
   ((alg-form? type)
    (alg-inhabited? (alg-form-to-name type)))
   ((arrow-form? type)
    (totally-inhabited? (arrow-form-to-val-type type)))
   ((star-form? type)
    (let ((left-type (star-form-to-left-type type))
	  (right-type (star-form-to-right-type type)))
      (and (totally-inhabited? left-type)
	   (totally-inhabited? right-type))))
   (else #f)))
