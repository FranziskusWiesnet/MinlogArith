	  ...
	  (let* ((proof (car proof-and-new-num-goals-and-maxgoal))
		...
		(list (make-proof-in-andnc-elim-right-form proof)
	       ...
	       (else (myerror
		      "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
		      "left or right expected" x1))))
	     ;changed 2025-08-21. total var has a totally-inhabited type
	     ((or (all-form? used-formula) (allnc-form? used-formula))
	      (let* ((var (car (quant-form-to-vars used-formula)))
		     (type (var-to-type var))
		     (t-deg (var-to-t-deg var))
		     (type1 (term-to-type x1))
		     (free1 (term-to-free x1))
		     (extra-vars (set-minus free1 (context-to-vars context))))
		(cond
		 ((not (term-form? x1))
		  (myerror "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
			   "term expected" x1))
		 ((pair? extra-vars)
		  (myerror "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
			   "variable in the context expected" (car extra-vars)))
		 ((not (type-le? type1 type))
		  (myerror "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
			   "types don't fit for all-elim" type1 type
			   "originating from all-formula" used-formula
			   "and use-with argument" x1))
		 ((and (t-deg-one? t-deg) (not (synt-total? x1)))
		  (myerror "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
			   "attempt to apply all-elim to non-total term" x1))
		 ((and (t-deg-one? t-deg) (not (type-to-total-inhab type1)))
		  (myerror "x-and-x-list-to-proof-and-new-num-goals-and-maxgoal"
			   "the type" type1
			   "of the syntactically total term" x1
			   "should be totally inhabited"))
		 (else (let ((x2 (if (equal? type type1)
                                     x1
                                     ((types-to-embedding type1 type) x1))))
			 (list (if (all-form? used-formula)
				   (make-proof-in-all-elim-form proof x2)
				   (make-proof-in-allnc-elim-form proof x2))
			       new-num-goals maxgoal))))))
	     ((atom-form? used-formula)
	      (cond
	       ((term=? (make-term-in-const-form
	...
	(let ((shortened-name-avar-alist ;only those with ToReal aconst
	 ...
	 (and (term-in-const-form? op)
		...
		(member name '("RealPlus" "RealUMinus" "RealMinus" "RealTimes"
			       ...
			       "In" "Out" "Av" "Va"
			       "RealMax" "RealMin"
			       "RealSum" "cRSum" "RealE" "cRE"))))))
      (let* ((op (term-in-app-form-to-final-op term))
	     (args (term-in-app-form-to-args term))
	     (name (const-to-name (term-in-const-form-to-const op))))
	(cond
	 ((member name '("RealUMinus" "RealAbs" "RealE" "cRE"))
...
;; (efproof) constructs a proof of the present goal from falsity, which should
...
(define (efproof)
  (let* ((num-goals (pproof-state-to-num-goals))
	 ...
	 (number
	  (num-goal-to-number
	   (if (pair? num-goals) (car num-goals) (myerror "goal missing"))))
	 (search-result (efproof-intern num-goals proof maxgoal)))
    (if (not search-result)
	(begin (display-comment "no proof found")
	       (if COMMENT-FLAG (newline)))
	(begin
...
(define (efproof-intern num-goals proof maxgoal)
  (let* ((num-goal
	  (if (pair? num-goals) (car num-goals) (myerror "goal missing")))
	 (goal (num-goal-to-goal num-goal))
	 (context (goal-to-context goal))
	 (formula (goal-to-formula goal))
 	 (proof1 (context-and-formula-to-efproof context formula)))
    (if (not proof1)
	#f
	(make-pproof-state (cdr num-goals)
			   (goal-subst proof goal proof1)
...
(define (context-and-formula-to-efproof context formula)
  (if (not (formula-of-nulltype? formula))
	(myerror "context-and-formula-to-efproof"
		 "n.c. goal expected" formula))
  (let* ((avars (context-to-avars context))
	 (falsity-avars
	  (list-transform-positive avars
	    (lambda (avar)
	      (formula=? (pf "F") (avar-to-formula avar))))))
    (if (pair? falsity-avars)
	(formula-and-falsity-avar-to-ef-proof formula (car falsity-avars))
	(myerror "context-and-formula-to-ef-proof"
		 "no falsity avar found in context"))))

;; Test
;; (set-goal "all n(F -> n=n)")
;; (assume "n" "Absurd")
;; (efproof)
;; (cp)
