...
\begin{acknowledgement*}
  ...
  \item Valentin Herrmann\index{Herrmann} (Minlogpad),
  ...
  \item Nils Köpp\index{Köpp} (many contributions, in particular
    corecursion, coinduction, lookahead for stream-represented real
    numbers),
  ...
  \item Klaus Weich\index{Weich} (proof search, the Fibonacci
    numbers example),
  \item Franziskus Wiesnet\index{Wiesnet} (constructive analysis with
    exact real numbers, documentation),
...
\subsection{efproof}
\texttt{efproof}\index{efproof@\texttt{efproof}} constructs a proof of
the present goal from falsity \texttt{F}, which must be part of the
context.

\subsection{def, defnc}
\texttt{def}\index{def@\texttt{def}} is used to introduce an
abbreviation of a term $t$ by a variable $x$ of the same type.  The
syntax is that def is called with two strings, the first for the
variable and the second for the term.  This adds a context item
\texttt{x=t} with name \texttt{xDef}.
\texttt{defnc}\index{defnc@\texttt{defnc}} does the same, with $x$ a
non-computational variable.
...
\nocite{KoeppSchwichtenberg23}
\nocite{SchwichtenbergSeisenbergerWiesnet16}
\nocite{SchwichtenbergWainer17}
\nocite{Schwichtenberg19}
\nocite{SchwichtenbergWiesnet21}
\nocite{Schwichtenberg23}
\nocite{Schwichtenberg23a}
\nocite{Wiesnet18}
