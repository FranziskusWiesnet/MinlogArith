...
(define (real-and-formula-to-mr-formula-aux real formula)
  (cond
     ...
     (let* ((prem (imp-form-to-premise formula))
	   ...
	   (let* ((var (type-to-new-partial-var type1))
		  ...
		  (prev-concl (real-and-formula-to-mr-formula-aux
			       appterm concl)))
	     (make-all var (make-imp prev-prem prev-concl))))))
   ((impnc-form? formula)
     (let ((prem (imp-form-to-premise formula))
	   (concl (imp-form-to-conclusion formula)))
       (make-imp prem (real-and-formula-to-mr-formula-aux real concl))))
   ...
   ((or (exca-form? formula) (excl-form? formula) (excu-form? formula))
...
(define (terms-to-eqp-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
	 ...
	 (if
	  ...
	  ;; (null? (type-to-tvars type))
	  (let* ((idpc-name
		  (string-append "EqP" (string-capitalize-first alg-name)))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name '() '())
	  ...
	  (let* ((idpc-name
		 ...
		 (types (alg-form-to-types type))
		 (prev-cterms (map type-to-eqp-cterm types))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
	 ...
	 (if
	  ...
	  (let* ((var (type-to-new-partial-var arg-type))
		 ...
		 (appterm2 (make-term-in-app-form term2 varterm))
		 (prev-concl
		  (terms-to-eqp-formula appterm1 appterm2)))
	    (make-all var (make-imp prem prev-concl)))
	  (let* ((var1 (type-to-new-partial-var arg-type))
		 ...
		 (varterm2 (make-term-in-var-form var2))
		 (appterm2 (make-term-in-app-form term2 varterm2))
		 (prev-prem (terms-to-eqp-formula varterm1 varterm2))
		 (prev-concl (terms-to-eqp-formula appterm1 appterm2)))
	    (mk-all var1 var2 (make-imp prev-prem prev-concl))))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      ...
	      (prev-left (terms-to-eqp-formula left-term1 left-term2))
	      (prev-right (terms-to-eqp-formula right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-eqp-formula" "type expected" type)))))

(define (type-to-eqp-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-eqp-formula varterm1 varterm2)))
...
(define (terms-to-eqpnc-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
	 ...
	 (if
	  (null? (alg-name-to-tvars alg-name))
	  (let* ((idpc-name
		  (string-append "EqP" (string-capitalize-first alg-name) "Nc"))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name '() '())
	 ...
	 (if
	  ...
	  (let* ((alg-name (alg-form-to-name arg-type))
		 ...
		 (prev-concl (terms-to-eqpnc-formula appterm1 appterm2)))
	    (make-allnc var1 (make-imp prem prev-concl)))
	  (let* ((var2 (type-to-new-partial-var arg-type))
		 (varterm2 (make-term-in-var-form var2))
		 (appterm2 (make-term-in-app-form term2 varterm2))
		 (prev-prem (terms-to-eqpnc-formula varterm1 varterm2))
		 (prev-concl (terms-to-eqpnc-formula appterm1 appterm2)))
	    (mk-all var1 var2 (make-imp prev-prem prev-concl))))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      ...
	      (left-term2 (make-term-in-lcomp-form term2))
	      (right-term2 (make-term-in-rcomp-form term2))
	      (prev-left (terms-to-eqpnc-formula left-term1 left-term2))
	      (prev-right (terms-to-eqpnc-formula right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-eqpnc-formula" "type expected" type)))))

(define (type-to-eqpnc-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-eqpnc-formula varterm1 varterm2)))
    (make-cterm var1 var2 fla)))
...
(define (term-to-ext-formula term)
  (let ((type (term-to-type term)))
    (case (tag type)
      ((tvar) (make-ext term))
      ((alg)
	 ...
	 (if
	  ...
	  (let* ((idpc-name
	    ...
	    (make-predicate-formula idpc term))
					;else alg name with tvars
	  (let* ((idpc-name
		  (string-append "RTotal"
				 (string-capitalize-first alg-name)))
		 (types (alg-form-to-types type))
		 (vars (map type-to-new-partial-var types))
		 (varterms (map make-term-in-var-form vars))
		 (flas (map (lambda (varterm)
			      (term-to-ext-formula varterm)) ;rec call
			    varterms))
	 ...
	 (cond
	  ((and (= 0 (type-to-level arg-type)) (null? (type-to-tvars arg-type)))
	   (let* ((var (type-to-new-var arg-type))
		  (varterm (make-term-in-var-form var))
		  (appterm (make-term-in-app-form term varterm))
		  (prev-concl (term-to-ext-formula appterm)))
	  ...
	  (else
	   (let* ((var1 (type-to-new-partial-var arg-type))
		  ...
		  (prev-prem (terms-to-eqp-formula
			      varterm1 varterm2 arg-type))
		  (prev-concl (terms-to-eqp-formula
			       appterm1 appterm2 val-type)))
	     (mk-all var1 var2 (make-imp prev-prem prev-concl)))))))
      ((star)
       (let* ((left-term (make-term-in-lcomp-form term))
	      (right-term (make-term-in-rcomp-form term))
	      (prev-left (term-to-ext-formula left-term))
	      (prev-right (term-to-ext-formula right-term)))
	 (make-and prev-left prev-right)))
      (else (myerror "term-to-ext-formula" "type expected" type)))))

(define (type-to-ext-cterm type)
  (let* ((var (type-to-new-partial-var type))
	 (varterm (make-term-in-var-form var))
	 (fla (term-to-ext-formula varterm)))
    (make-cterm var fla)))

(define (term-to-extnc-formula term)
  (let ((type (term-to-type term)))
    (case (tag type)
	   ...
	   (let* ((idpc-name
		  ...
		  (idpc (if
			 ...
			 (myerror "term-to-extnc-formula"
				  "unknown idpredconst name" idpc-name))))
	     (make-predicate-formula idpc term)))
	  ((= (type-to-level type) 0)
	   (let* ((idpc-name (string-append
			      "RTotal" (string-capitalize-first alg-name)
			      "Nc"))
		  (types (alg-form-to-types type))
		  (prev-cterms (map type-to-extnc-cterm types))
		  (idpc (if
			 (assoc idpc-name IDS)
			 (idpredconst-name-and-types-and-cterms-to-idpredconst
			  idpc-name types prev-cterms)
			 (myerror "term-to-extnc-formula"
				  "unknown idpredconst name" idpc-name))))
	     (make-predicate-formula idpc term)))
	  (else (terms-to-eqp-formula term term)))))
      ((arrow)
       (let ((arg-type (arrow-form-to-arg-type type)))
	 (if
	  (alg-form? arg-type)
	  (let* ((var (type-to-new-partial-var arg-type))
		 (varterm (make-term-in-var-form var))
		 (appterm (make-term-in-app-form term varterm))
		 (prev-prem (term-to-extnc-formula varterm))
		 (prev-concl (term-to-extnc-formula appterm)))
	    (mk-all var (make-imp prev-prem prev-concl)))
	  (terms-to-eqp-formula term term))))
      ((star)
       (let* ((left-term (make-term-in-lcomp-form term))
	      (right-term (make-term-in-rcomp-form term))
	      (prev-left (term-to-extnc-formula left-term))
	      (prev-right (term-to-extnc-formula right-term)))
	 (make-and prev-left prev-right)))
      (else (myerror "term-to-extnc-formula" "type expected" type)))))

(define (type-to-extnc-cterm type)
  (let* ((var (type-to-new-partial-var type))
	 (varterm (make-term-in-var-form var))
	 (fla (term-to-extnc-formula varterm type)))
    (make-cterm var fla)))
...
(define (terms-to-pure-eqp-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
      ((tvar) (make-eqp term1 term2))
      ((alg)
	 ...
	 (if
	  ...
	  (let* ((idpc-name
		  (string-append "REqP" (string-capitalize-first alg-name)))
		 (types (alg-form-to-types type))
		 (prev-cterms (map type-to-pure-eqp-cterm types))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (varterm2 (make-term-in-var-form var2))
	      (appterm2 (make-term-in-app-form term2 varterm2))
	      (prev-prem (terms-to-pure-eqp-formula varterm1 varterm2))
	      (prev-concl (terms-to-pure-eqp-formula appterm1 appterm2)))
	 (mk-all var1 var2 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      (right-term1 (make-term-in-rcomp-form term1))
	      (left-term2 (make-term-in-lcomp-form term2))
	      (right-term2 (make-term-in-rcomp-form term2))
	      (prev-left (terms-to-pure-eqp-formula left-term1 left-term2))
	      (prev-right (terms-to-pure-eqp-formula right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-eqp-formula" "type expected" type)))))

(define (type-to-pure-eqp-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-pure-eqp-formula varterm1 varterm2)))
    (make-cterm var1 var2 fla)))

(define (term-to-pure-ext-formula term)
...
(define (terms-to-pure-eqpnc-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
      ((tvar) (make-eqpnc term1 term2))
      ((alg)
	 ...
	 (if
	  ...
	  (let* ((idpc-name ;Code discarded 2020-07-02
	  	  (string-append "REqP"
	  			 ...
	  			 "Nc"))
	  	 (types (alg-form-to-types type))
	  	 (prev-cterms (map type-to-pure-eqpnc-cterm types))
	  	 (idpc (if
	  		(assoc idpc-name IDS)
	  		(idpredconst-name-and-types-and-cterms-to-idpredconst
	  		 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (varterm2 (make-term-in-var-form var2))
	      (appterm2 (make-term-in-app-form term2 varterm2))
	      (prev-prem (terms-to-pure-eqpnc-formula varterm1 varterm2))
	      (prev-concl (terms-to-pure-eqpnc-formula appterm1 appterm2)))
	 (mk-all var1 var2 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-type (star-form-to-left-type type))
	      ...
	      (prev-left (terms-to-pure-eqpnc-formula
			  left-term1 left-term2))
	      (prev-right (terms-to-pure-eqpnc-formula
			   right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-eqpnc-formula" "type expected" type)))))

(define (type-to-pure-eqpnc-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-pure-eqpnc-formula varterm1 varterm2)))
    (make-cterm var1 var2 fla)))
...
(define (terms-to-pure-mr-eqp-formula term1 term2 real)
  (let ((type (term-to-type term1)))
    (if (not (equal? type (term-to-type term1)))
	 ...
	 (if
	  ...
	  (let* ((idpc-name
		  (string-append "REqP"
				 ...
				 "MR"))
		 (types (alg-form-to-types type))
		 (prev-cterms (map type-to-pure-mr-eqp-cterm types))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (prev-prem (terms-to-pure-mr-eqp-formula
			  varterm1 varterm2 varterm3))
	      (prev-concl (terms-to-pure-mr-eqp-formula
			   appterm1 appterm2 appterm3)))
	 (mk-all var1 var2 var3 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      (right-term1 (make-term-in-rcomp-form term1))
	      (left-term2 (make-term-in-lcomp-form term2))
	      ...
	      (prev-left (terms-to-pure-mr-eqp-formula
			  left-term1 left-term2 left-real))
	      (prev-right (terms-to-pure-mr-eqp-formula
			   right-term1 right-term2 right-real)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-mr-eqp-formula"
		     "type expected" type)))))

(define (type-to-pure-mr-eqp-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 ...
	 (varterm3 (make-term-in-var-form var3))
	 (fla (terms-to-pure-mr-eqp-formula varterm1 varterm2 varterm3)))
    (make-cterm var1 var2 var3 fla)))

;; 2025-10-09.  Added terms-to-pure-coeqp-formula , term-to-pure-coext-formula
;; terms-to-pure-coeqpnc-formula , term-to-pure-coextnc-formula

(define (terms-to-pure-coeqp-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
      ((tvar) (make-coeqp term1 term2))
      ((alg)
       (let ((alg-name (alg-form-to-name type)))
	 (if
	  (null? (alg-name-to-tvars alg-name))
	  (let* ((idpc-name
		 ...
		 (idpc (if
			...
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name '() '())
			(myerror "terms-to-pure-coeqp-formula"
				 "unknown idpredconst name" idpc-name))))
	    (make-predicate-formula idpc term1 term2))
					;else alg name with tvars
	  (let* ((idpc-name
		  (string-append "REqP"
				 (string-capitalize-first alg-name)))
		 (types (alg-form-to-types type))
		 (prev-cterms (map type-to-pure-coeqp-cterm types))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (varterm2 (make-term-in-var-form var2))
	      (appterm2 (make-term-in-app-form term2 varterm2))
	      (prev-prem (terms-to-pure-coeqp-formula varterm1 varterm2))
	      (prev-concl (terms-to-pure-coeqp-formula appterm1 appterm2)))
	 (mk-all var1 var2 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      (right-term1 (make-term-in-rcomp-form term1))
	      (left-term2 (make-term-in-lcomp-form term2))
	      (right-term2 (make-term-in-rcomp-form term2))
	      (prev-left (terms-to-pure-coeqp-formula left-term1 left-term2))
	      (prev-right (terms-to-pure-coeqp-formula right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-coeqp-formula" "type expected" type)))))

(define (type-to-pure-coeqp-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-pure-coeqp-formula varterm1 varterm2)))
    (make-cterm var1 var2 fla)))

(define (term-to-pure-coext-formula term)
  (terms-to-pure-coeqp-formula term term))
...
(define (terms-to-pure-coeqpnc-formula term1 term2)
  (let ((type (term-to-type term1)))
    (case (tag type)
      ((tvar) (make-coeqpnc term1 term2))
      ((alg)
       (let* ((alg-name (alg-form-to-name type)))
	 (if
	  (null? (alg-name-to-tvars alg-name))
	  (let* ((idpc-name
		 ...
		 (idpc (if
			...
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name '() '())
			(myerror "terms-to-pure-coeqpnc-formula"
				 "unknown idpredconst name" idpc-name))))
	    (make-predicate-formula idpc term1 term2))
					;else alg name with tvars
	  (let* ((idpc-name ;Code discarded 2020-07-02
	  	  (string-append "REqP"
	  			 ...
	  			 "Nc"))
	  	 (types (alg-form-to-types type))
	  	 (prev-cterms (map type-to-pure-coeqpnc-cterm types))
	  	 (idpc (if
	  		(assoc idpc-name IDS)
	  		(idpredconst-name-and-types-and-cterms-to-idpredconst
	  		 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (varterm2 (make-term-in-var-form var2))
	      (appterm2 (make-term-in-app-form term2 varterm2))
	      (prev-prem (terms-to-pure-coeqpnc-formula varterm1 varterm2))
	      (prev-concl (terms-to-pure-coeqpnc-formula appterm1 appterm2)))
	 (mk-all var1 var2 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-type (star-form-to-left-type type))
	      (right-type (star-form-to-right-type type))
	      ...
	      (left-term2 (make-term-in-lcomp-form term2))
	      (right-term2 (make-term-in-rcomp-form term2))
	      (prev-left (terms-to-pure-coeqpnc-formula
			  left-term1 left-term2))
	      (prev-right (terms-to-pure-coeqpnc-formula
			   right-term1 right-term2)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-coeqpnc-formula" "type expected" type)))))

(define (type-to-pure-coeqpnc-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (fla (terms-to-pure-coeqpnc-formula varterm1 varterm2)))
    (make-cterm var1 var2 fla)))

(define (term-to-pure-coextnc-formula term)
  (terms-to-pure-coeqpnc-formula term term))
...
(define (terms-to-pure-mr-coeqp-formula term1 term2 real)
  (let ((type (term-to-type term1)))
    (if (not (equal? type (term-to-type term1)))
	(myerror "terms-to-pure-mr-coeqp-formula"
		 "terms of equal type expected"
		 term1 real
		 "which have types"
		 (term-to-type term1) type))
	...
	(myerror "terms-to-pure-mr-coeqp-formula"
		 ...
		 (term-to-type term2) type))
    (case (tag type)
      ((tvar) (make-coeqpmr term1 term2 real))
      ((alg)
       (let* ((alg-name (alg-form-to-name type)))
	 (if
	  (null? (alg-name-to-tvars alg-name))
	  ...
	  (let* ((idpc-name
		  (string-append "REqP"
				 ...
				 "MR"))
		 (types (alg-form-to-types type))
		 (prev-cterms (map type-to-pure-mr-coeqp-cterm types))
		 (idpc (if
			(assoc idpc-name IDS)
			(idpredconst-name-and-types-and-cterms-to-idpredconst
			 idpc-name types prev-cterms)
      ...
      ((arrow)
       (let* ((arg-type (arrow-form-to-arg-type type))
	      ...
	      (varterm3 (make-term-in-var-form var3))
	      (appterm3 (make-term-in-app-form term3 varterm3))
	      (prev-prem (terms-to-pure-mr-coeqp-formula
			  varterm1 varterm2 varterm3))
	      (prev-concl (terms-to-pure-mr-coeqp-formula
			   appterm1 appterm2 appterm3)))
	 (mk-all var1 var2 var3 (make-imp prev-prem prev-concl))))
      ((star)
       (let* ((left-term1 (make-term-in-lcomp-form term1))
	      ...
	      (left-real (make-term-in-lcomp-form real))
	      (right-real (make-term-in-rcomp-form real))
	      (prev-left (terms-to-pure-mr-coeqp-formula
			  left-term1 left-term2 left-real))
	      (prev-right (terms-to-pure-mr-coeqp-formula
			   right-term1 right-term2 right-real)))
	 (make-and prev-left prev-right)))
      (else (myerror "terms-to-pure-mr-coeqp-formula"
		     "type expected" type)))))

(define (type-to-pure-mr-coeqp-cterm type)
  (let* ((var1 (type-to-new-partial-var type))
	 (varterm1 (make-term-in-var-form var1))
	 (var2 (type-to-new-partial-var type))
	 (varterm2 (make-term-in-var-form var2))
	 (var3 (type-to-new-partial-var type))
	 (varterm3 (make-term-in-var-form var3))
	 (fla (terms-to-pure-mr-coeqp-formula varterm1 varterm2 varterm3)))
    (make-cterm var1 var2 var3 fla)))

;; End of additions 2025-10-09,
...
(define (add-co idpc-name . opt-prim-prod-eq-info)
  ...
  (let*
	...
	(map (lambda (transposed-argss)
	       (map (lambda (terms)
		      (let ((t-degs (map var-to-t-deg
					 (apply union
						(map term-to-free terms)))))
			(if (null? t-degs)
			    t-deg-zero
			    (apply min t-degs))))
		    transposed-argss))
	...
	(map (lambda (vars clause-with-fvars)
	       (apply (if nc-idpc? mk-all mk-allnc)
	...
	((all-allnc-form? fla)
	 (let* ((var (all-allnc-form-to-var fla))
	   ...
	   (cond ((or nc-idpc? (formula-of-nulltype? prev)) (mk-exnc var prev))
		 ((or (allnc-form? fla) (t-deg-zero? (var-to-t-deg var)))
		  (mk-exr var prev))
