...
(define (animate thm-or-ga-name . opt-eterm)
  (let ((pconst-name
    ...
    (if (pair? (pconst-name-to-comprules pconst-name))
	(myerror "animate" "already animated" thm-or-ga-name))
	 ...
	 (if
	  ...
	  (let* ((eterm (if (pair? opt-eterm)
	    ...
	    (add-computation-rule
	     (make-term-in-const-form pconst) eterm))))))))
...
(define (animate-no-nt thm-or-ga-name . opt-eterm)
  (let* ((pconst-name
	  (theorem-or-global-assumption-name-to-pconst-name thm-or-ga-name))
	 (pconst (pconst-name-to-pconst pconst-name))
	 (info1 (assoc thm-or-ga-name THEOREMS)))
...
(define (animate-bpe-nt thm-or-ga-name . opt-eterm)
  (let* ((pconst-name
	  (theorem-or-global-assumption-name-to-pconst-name thm-or-ga-name))
	 (pconst (pconst-name-to-pconst pconst-name))
	 (info1 (assoc thm-or-ga-name THEOREMS)))
    (if
     ...
     (let* ((proof (theorem-name-to-proof thm-or-ga-name))
	    (eterm (proof-to-extracted-term proof))
	    (bpe-neterm (bpe-nt eterm)))
       (add-computation-rule (make-term-in-const-form pconst) bpe-neterm))
     (let ((info2 (assoc thm-or-ga-name GLOBAL-ASSUMPTIONS)))
       (if
	info2
	(let* ((eterm (if (pair? opt-eterm)
	...
	(lambda (name)
      ...
      (let* ((cr-thm-names
		...
		(lambda (name)
	     ...
	     (pconst-names
	      (map theorem-or-global-assumption-name-to-pconst-name
		   cr-thm-names)))
	(list-transform-positive pconst-names
	  (lambda (name) (pair? (pconst-name-to-comprules name)))))))

(define (deanimate-all)
  (let* ((rel-pconsts
	  (list-transform-positive PROGRAM-CONSTANTS
	    (lambda (x)
	      (let ((name (car x)))
		(and (string=? "c" (substring name 0 1))
		     (assoc (substring name 1 (string-length name)) THEOREMS)
		     ;; (not (final-substring? "Clause" name))
		     ;; (not (final-substring? "Intro" name))
		     ;; (not (final-substring? "Elim" name))
		     (pair? (pconst-name-to-comprules name)))))))
	 (cthm-names
	  (map (lambda (x) (let ((name (car x)))
			     (substring name 1 (string-length name))))
	       rel-pconsts)))
    (do ((l cthm-names (cdr l)))
	((null? l) *the-non-printing-object*)
      (deanimate (car l)))))

(define (cthm-name-to-cthm-names name)
  (let* ((proof (theorem-name-to-proof name))
	 (eterm (proof-to-extracted-term proof))
	 (consts (term-to-consts eterm))
	 (cthm-consts
	  (list-transform-positive consts
	    (lambda (const)
	      (let ((str (const-to-name const)))
		(string=? "c" (substring str 0 1))))))
	 (pconst-names (map const-to-name cthm-consts))
	 (cthm-names
	  (map (lambda (x) (substring x 1 (string-length x))) pconst-names)))
    (remove-duplicates cthm-names)))

(define (dep-tree cthm-name)
  (let ((cthm-names (cthm-name-to-cthm-names cthm-name)))
    (cons cthm-name (map dep-tree cthm-names))))

;; To avoid repetitions of already expanded dependence trees we use

(define (dep-tree-aux cthm-name done)
  (if (member cthm-name done)
      (list cthm-name)
      (let* ((cthm-names (cthm-name-to-cthm-names cthm-name))
	     (new-done (cons cthm-name done)))
	(cons cthm-name (map (lambda (str) (dep-tree-aux str new-done))
			     cthm-names)))))
...
;; EqDTrans EqDCompatApp EqDCompatApp2 EFEqD are proven and added to
;; THEOREMS
...
;; In proof-to-extracted-term proof-to-extracted-term-aux replaced by
;; proof-to-extracted-term-aux1 .  In case the type of eterm is of
;; yprod-form and (term-in-pair-form-wrt-yprod? eterm) is false, then
;; proof-to-extracted-term-aux1 changes eterm into
;; (term-to-term-in-pair-form-wrt-yprod eterm).  Else eterm is kept.
     ...
     (if
	  ...
	  (let*  ((proof-or-thm-name first)
		...
		(let ((avar-to-var (make-avar-to-var)))
		  (proof-to-extracted-term-aux1 proof avar-to-var #f))))
					;else first is the unfold-let-flag
	  (let ((avar-to-var (make-avar-to-var)))
	    (proof-to-extracted-term-aux1 (current-proof) avar-to-var first)))
		...
		(theorem-to-extracted-term
		 ...
		 unfold-let-flag))
	       (else (let ((avar-to-var (make-avar-to-var)))
		       (proof-to-extracted-term-aux1
			proof avar-to-var unfold-let-flag)))))
	    (myerror "proof-to-extracted-term"
		     "proof or theorem name expected"
		     first)))))))

(define (proof-to-extracted-term-aux1 proof avar-to-var unfold-let-flag)
  (let ((eterm (proof-to-extracted-term-aux
		proof avar-to-var unfold-let-flag)))
    (if (and (yprod-form? (term-to-type eterm))
	     (not (term-in-pair-form-wrt-yprod? eterm)))
	(term-to-term-in-pair-form-wrt-yprod eterm)
	eterm)))
  ...
  (case (tag proof)
    ...
    ((proof-in-all-elim-form)
     (let* ((op (proof-in-all-elim-form-to-op proof))
	    ...
	    (all-fla (proof-to-formula op))
	    (t-deg (var-to-t-deg (all-form-to-var all-fla)))
	    (eterm-op
	     (proof-to-extracted-term-aux op avar-to-var unfold-let-flag)))
       (if (t-deg-zero? t-deg)
	   eterm-op
	   (make-term-in-app-form eterm-op arg))))
    ;; ((proof-in-all-elim-form)
    ;;  (let* ((op (proof-in-all-elim-form-to-op proof))
    ;; 	    (arg (proof-in-all-elim-form-to-arg proof))
    ;; 	    (eterm-op
    ...
    ;; 	   eterm-op
    ;; 	   (make-term-in-app-form eterm-op arg))))
    ((proof-in-allnc-intro-form) ;obsolete
     (let ((kernel (proof-in-allnc-intro-form-to-kernel proof)))
       (proof-to-extracted-term-aux kernel avar-to-var unfold-let-flag)))
    ((proof-in-allnc-elim-form) ;obsolete
...
(define (theorem-to-extracted-term aconst . opt-unfold-let-flag)
  ...
  (if
   ...
   (let* ((unfold-let-flag (car opt-unfold-let-flag))
     ...
     (cond
      ((string=? "InhabTotal" thm-name) ;obsolete
...
(add-ids (list (list "EqD" (make-arity (py "alpha") (py "alpha"))))
	 '("all alpha^ EqD alpha^ alpha^" "InitEqD"))

(add-ids (list (list "ExR" (make-arity) "identity"))
	 '("all alpha^((Pvar alpha)alpha^ -> ExR)" "InitExR"))

(add-ids (list (list "ExNc" (make-arity)))
	 '("all alpha^((Pvar alpha)^alpha^ -> ExNc)" "InitExNc"))
...
;; PairConstrExtNc
(set-goal "all alpha1^1,alpha1^2(EqPNc alpha1^1 alpha1^2 ->
 all alpha2^1,alpha2^2(EqPNc alpha2^1 alpha2^2 ->
 (REqPYprodNc
...
;; all (alpha1 yprod alpha2)^,(alpha1 yprod alpha2)^0(
...
;;      all (alpha1=>alpha2=>beta)^1,(alpha1=>alpha2=>beta)^2(
;;       all alpha1^3,alpha1^4(
;;        EqPNc alpha1^3 alpha1^4 -> 
;;        all alpha2^5,alpha2^6(
...
;; all (alpha1 ysum alpha2)^,(alpha1 ysum alpha2)^0(
;;      (REqPYsumNc (cterm (alpha1^1,alpha1^2) EqPNc alpha1^1 alpha1^2)
;;        (cterm (alpha2^1,alpha2^2) EqPNc alpha2^1 alpha2^2))
;;      (alpha1 ysum alpha2)^
;;      (alpha1 ysum alpha2)^0 -> 
;;      all (alpha1=>beta)^1,(alpha1=>beta)^2(
;;       all alpha1^3,alpha1^4(
;;        EqPNc alpha1^3 alpha1^4 -> 
;;        EqPNc((alpha1=>beta)^1 alpha1^3)((alpha1=>beta)^2 alpha1^4)) -> 
;;       all (alpha2=>beta)^3,(alpha2=>beta)^4(
;;        all alpha2^5,alpha2^6(
;;         EqPNc alpha2^5 alpha2^6 -> 
...
(assert "all (alpha1=>beta)^,(alpha1=>beta)^0(
     all alpha1^1,alpha1^2(
      EqPNc alpha1^1 alpha1^2 -> 
      EqPNc((alpha1=>beta)^ alpha1^1)((alpha1=>beta)^0 alpha1^2)) -> 
     all (alpha2=>beta)^1,(alpha2=>beta)^2(
      all alpha2^3,alpha2^4(
       EqPNc alpha2^3 alpha2^4 -> 
       EqPNc((alpha2=>beta)^1 alpha2^3)((alpha2=>beta)^2 alpha2^4)) -> 
      all (alpha1 ysum alpha2)^3,(alpha1 ysum alpha2)^4(
  ...
  (let* ((type (make-alg "boole"))
    ...
    (mk-proof-in-intro-form
     var1 u1 (mk-proof-in-elim-form
	      ...
	      (mk-proof-in-intro-form ;all q^(q^ =True -> p^ =True)
...
;; We need to (re)define proof-to-extracted-term-aux here to correctly
;; extract a term from eqd-compat-proof .
  ...
  (case (tag proof)
    ...
    ((proof-in-all-elim-form)
     (let* ((op (proof-in-all-elim-form-to-op proof))
	    ...
	    (all-fla (proof-to-formula op))
	    (t-deg (var-to-t-deg (all-form-to-var all-fla)))
	    ;; (t-deg (all-form-to-t-deg all-fla))
	    (eterm-op
	     (proof-to-extracted-term-aux op avar-to-var unfold-let-flag)))
       (if (t-deg-zero? t-deg)
	   eterm-op
  ...
  (let* ((aconst (theorem-name-to-aconst "EfEqD"))
	 ...
	 (elim-proof2
	  (mk-proof-in-elim-form
	   ...
	   (make-term-in-var-form var)
	   elim-proof1)))
    (mk-proof-in-intro-form  falsity-avar var elim-proof2)))
...
;; For totally inhabited types we can use total variables.  Example:

;; StabAllBoole
(set-goal
 "all boole((((Pvar boole)^ boole -> F) -> F) -> (Pvar boole)^ boole) ->
 ((all boole(Pvar boole)^ boole -> F) -> F) -> all boole(Pvar boole)^ boole")
(assume "StabHyp" "NotNotAll" "boole")
(use "StabHyp")
(assume "NotP")
(use "NotNotAll")
(assume "AllP")
...
;; StabAllImpBoole
(set-goal
 "all boole((((Pvar boole)^2 boole -> F) -> F) -> (Pvar boole)^2 boole) ->
 ((all boole((Pvar boole)^1 boole -> (Pvar boole)^2 boole) -> F) -> F) ->
 all boole((Pvar boole)^1 boole -> (Pvar boole)^2 boole)")
(assume "StabHyp" "NotNotAllImp" "boole" "P1a")
(use "StabHyp")
(assume "NotP2a")
(use "NotNotAllImp")
(assume "AllImp")
  ...
  (let* ((tvar (make-tvar -1 DEFAULT-TVAR-NAME))
	 ...
	 (expartial-fla
	  (mk-exnc varpartial
		  (mk-andnc (make-totalnc varpartialterm)
			    (make-predicate-formula pvar varpartialterm))))
	 (formula-of-exnctotal-elim-aconst (mk-imp exnc-fla expartial-fla)))
    (make-aconst "ExNcTotalElim"
		 'axiom formula-of-exnctotal-elim-aconst empty-subst)))

(add-theorem "ExNcTotalIntro"
	     (make-proof-in-aconst-form exnctotal-intro-aconst))
(add-theorem "ExNcTotalElim"
	     (make-proof-in-aconst-form exnctotal-elim-aconst))

(pp "ExNcTotalIntro")

;; exnc alpha^(TotalNc alpha^ andnc (Pvar alpha)^' alpha^) -> 
...
;; The following commented out code is obsolete.
  ...
  (let* ((bvar1 (make-var (make-alg "boole") 1 0 ""))
    ...
    (make-proof-in-all-intro-form
     bvar1 (make-proof-in-imp-intro-form
	    u1 (make-proof-in-all-intro-form
...
;; (pp (proof-to-formula and-const-total-proof))

;; all boole^1(
;;  TotalBoole boole^1 -> 
;;  all boole^2(TotalBoole boole^2 -> TotalBoole(boole^1 andb boole^2)))
  ...
  (let* ((bvar1 (make-var (make-alg "boole") 1 0 ""))
    ...
    (mk-proof-in-intro-form
     bvar1 (mk-proof-in-intro-form
	    u1 (mk-proof-in-intro-form
		bvar2 elim-proof)))))

;; (pp (proof-to-formula imp-const-total-proof))

;; all boole^1(
;;  TotalBoole boole^1 -> 
;;  all boole^2(TotalBoole boole^2 -> TotalBoole(boole^1 impb boole^2)))
...
;; (pp (proof-to-formula or-const-total-proof))

;; all boole^1(
;;  TotalBoole boole^1 -> 
;;  all boole^2(TotalBoole boole^2 -> TotalBoole(boole^1 orb boole^2)))
...
;; (pp (proof-to-formula neg-const-total-proof))

;; all boole^1(TotalBoole boole^1 -> TotalBoole(negb boole^1))

(add-theorem "NegConstTotal" neg-const-total-proof)

;; AndConstEqP
(set-goal "all boole^1,boole^2,boole^3,boole^4(
...
;; NegConstEqP
(set-goal "all boole^1,boole^2(EqPBoole boole^1 boole^2 ->
...
;; (pp (proof-to-formula boole-if-total-proof))

;; all boole^(
;;  TotalBoole boole^ -> 
;;  all alpha^1,alpha^2(
;;   Total alpha^1 -> Total alpha^2 -> Total[if boole^ alpha^1 alpha^2]))
...
;; (pp (proof-to-formula boole-eq-total-proof))

;; all boole^1(
;;  TotalBoole boole^1 -> 
;;  all boole^2(TotalBoole boole^2 -> TotalBoole(boole^1=boole^2)))
...
;; (pp (proof-to-formula boole-total-var-proof))
;; all boole907 TotalBoole boole907
...
(pp (proof-to-formula yprod-total-var-proof))
;; all (alpha1 yprod alpha2)_917 TotalYprod(alpha1 yprod alpha2)_917
...
(pp (proof-to-formula ysum-total-var-proof))
;; all (alpha1 ysum alpha2)_927 TotalYsum(alpha1 ysum alpha2)_927
...
;; TotalToPairLftRhtEq
(set-goal "all (alpha1 yprod alpha2)^(
...
(add-ids
 ...
 '("all alpha1^,alpha2^ STotalYprod(alpha1^ pair alpha2^)" "InitSTotalYprod"))
...
(add-ids
 ...
 '("all alpha1^ STotalYsum((InL alpha1 alpha2)alpha1^)" "STotalYsumInL")
 '("all alpha2^ STotalYsum((InR alpha2 alpha1)alpha2^)" "STotalYsumInR"))

(add-ids
 (list (list "STotalYsumNc" (make-arity (py "alpha1 ysum alpha2"))))
 '("all alpha1^ STotalYsumNc((InL alpha1 alpha2)alpha1^)" "STotalYsumNcInL")
 '("all alpha2^ STotalYsumNc((InR alpha2 alpha1)alpha2^)" "STotalYsumNcInR"))
...
;; BooleIfEqPNc
(set-goal
"all boole^1,boole^2(EqPBooleNc boole^1 boole^2 ->
 all alpha^11,alpha^21(EqPNc alpha^11 alpha^21 ->
 all alpha^12,alpha^22(EqPNc alpha^12 alpha^22 ->
...
;; BooleIfEqP
(set-goal
"all boole^1,boole^2(EqPBoole boole^1 boole^2 ->
 all alpha^11,alpha^21(EqP alpha^11 alpha^21 ->
 all alpha^12,alpha^22(EqP alpha^12 alpha^22 ->
...
;; EqPBooleRefl
(set-goal "all boole^(TotalBoole boole^ -> EqPBoole boole^ boole^)")
(assume "boole^" "Tb")
(elim "Tb")
(use "EqPBooleTrue")
(use "EqPBooleFalse")
...
;; EqPBooleNcRefl
(set-goal "all boole^(TotalBooleNc boole^ -> EqPBooleNc boole^ boole^)")
(assume "boole^" "Tb")
(elim "Tb")
(use "EqPBooleNcTrue")
(use "EqPBooleNcFalse")
...
;; EqPBooleSym
(set-goal
 "all boole^1,boole^2(EqPBoole boole^1 boole^2 -> EqPBoole boole^2 boole^1)")
(assume "boole^1" "boole^2" "EqPp1p2")
(elim "EqPp1p2")
(use "EqPBooleTrue")
(use "EqPBooleFalse")
...
(set-goal "all boole^1,boole^2(
 EqPBoole boole^1 boole^2 -> all boole^3(EqPBoole boole^2 boole^3 ->
...
(set-goal
 "all boole^1,boole^2(EqPBooleNc boole^1 boole^2 -> TotalBooleNc boole^1)")
(assume "boole^1" "boole^2" "EqPb1b2")
(elim "EqPb1b2")
...
(set-goal
 "all boole^1,boole^2(EqPBooleNc boole^1 boole^2 -> TotalBooleNc boole^2)")
(assume "boole^1" "boole^2" "EqPb1b2")
(elim "EqPb1b2")
(use "TotalBooleNcTrue")
(use "TotalBooleNcFalse")
...
;; BooleEqToEqPNc
(set-goal "all boole^1(TotalBooleNc boole^1 ->
 all boole^2(TotalBooleNc boole^2 -> boole^1=boole^2 ->
...
;; EqDCompatApp2
(set-goal "all (alpha=>beta)^1,(alpha=>beta)^2,alpha^1,alpha^2(
 (alpha=>beta)^1 eqd(alpha=>beta)^2 ->
 alpha^1 eqd alpha^2 -> (alpha=>beta)^1 alpha^1 eqd (alpha=>beta)^2 alpha^2)")
(assume "(alpha=>beta)^1" "(alpha=>beta)^2" "alpha^1" "alpha^2" "fEq" "opEq")
(simp "fEq")
(simp "opEq")
(use "InitEqD")
;; Proof finished.
...
;; EqPYprodPairOne
(set-goal "all (alpha1 yprod alpha2)^1,(alpha1 yprod alpha2)^2(
...
;; YprodIfTotal
(set-goal "all (alpha yprod beta)^(TotalYprod (alpha yprod beta)^ ->
 all (alpha=>beta=>gamma)^(
  all alpha^(Total alpha^ -> all beta^(Total beta^ -> 
...
(set-goal "all boole^(TotalBoole boole^ -> TotalBooleNc boole^)")
(assume "boole^" "Tb")
(use "Tb")
;; Proof finished.
;; (cp)
...
;; EqPToEqPNc
(set-goal "all alpha^1,alpha^2(EqP alpha^1 alpha^2 -> EqPNc alpha^1 alpha^2)")
...
(define (andlr-formula-and-concl-to-andlr-elim-mr-proof andlr-formula concl)
  (let* ((andlr-type (formula-to-et-type andlr-formula))
	 ...
	 (concl-type (formula-to-et-type concl))
	 (f-var (type-to-new-partial-var (make-arrow andlr-type concl-type)))
	 (f-term (make-term-in-var-form f-var))
	 ...
	 (vars (all-allnc-form-to-vars elim-aconst-fla))
	 ...
	 (intro-proof-f (make-proof-in-all-intro-form f-var elim-proof))
	 (intro-proof-avar (make-proof-in-imp-intro-form
			    mr-prem-avar intro-proof-f))
	 (rest-vars (remove f-var vars)))
    (apply mk-proof-in-intro-form
	   (append rest-vars (list intro-proof-avar)))))

;; Code discarded 2029-01-29
;; (define (andlr-formula-and-concl-to-andlr-elim-mr-proof andlr-formula concl)
;;   (let* ((andlr-type (formula-to-et-type andlr-formula))
;; 	 (z (type-to-new-partial-var andlr-type))
;; 	 (z-term (make-term-in-var-form z))
...
;; Commented out 2026-02-07.  Not used any more.
;; Added 2026-02-01
;; (define (andd-formula-and-concl-to-andd-elim-mr-proof andd-formula concl)
;;   (let* ((andd-type (formula-to-et-type andd-formula))
;; 	 (z (type-to-new-partial-var andd-type))
;; 	 (z-term (make-term-in-var-form z))
;; 	 (mr-andd-fla ;z mr A andd B
;; 	  (real-and-formula-to-mr-formula-aux z-term andd-formula))
;; 	 (concl-type (formula-to-et-type concl))
;; 	 (f-var (type-to-new-partial-var (make-arrow andd-type concl-type)))
;; 	 (f-term (make-term-in-var-form f-var))
;; 	 (app-term (make-term-in-app-form f-term z-term)) ;fz
;; 	 (mr-concl ;fz mr concl
;; 	  (real-and-formula-to-mr-formula-aux app-term concl))
;; 	 (imp-fla (make-imp mr-andd-fla mr-concl))
;; 	 (elim-aconst (imp-formulas-to-elim-aconst imp-fla))
;; 	 (elim-aconst-fla (aconst-to-formula elim-aconst))
;; 	 (vars (all-allnc-form-to-vars elim-aconst-fla))
;; 	 (elim-vars-proof (apply mk-proof-in-elim-form
...
;; 		      elim-vars-proof (make-proof-in-avar-form mr-prem-avar)))
;; 	 (intro-proof-f (make-proof-in-all-intro-form f-var elim-proof))
;; 	 (intro-proof (make-proof-in-imp-intro-form
;; 		       mr-prem-avar intro-proof-f))
;; 	 (rest-vars (remove f-var vars))
;; 	 (pair-var (car (reverse rest-vars)))
;; 	 (idpc (make-idpredconst
;; 		"STotalYprod"
;; 		(list (yprod-form-to-left-type andd-type)
;; 		      (yprod-form-to-right-type andd-type))
;; 		'()))
;; 	 (stotal-fla
;; 	  (make-predicate-formula idpc (make-term-in-var-form pair-var)))
;; 	 (stotal-avar (formula-to-new-avar stotal-fla)))
;;     (apply mk-proof-in-intro-form
;; 	   (append rest-vars (list stotal-avar intro-proof)))))
  ...
  (let* ((info (assoc thm-name THEOREMS))
     ...
     *the-non-printing-object*
     (let* (;(eproof (expand-theorems proof (lambda (x) (string=? "Id" x))))
	    ...
	    (eterm (proof-to-extracted-term proof))
	    (neterm (rename-variables (nt eterm))))
       (if (not (assoc pconst-name PROGRAM-CONSTANTS))
	   (add-program-constant-silent pconst-name uninst-type))
       (add-computation-rule-silent
	...
	neterm)
	 ...
	 (let* ((saconst (make-aconst sname 'theorem new-sfla empty-subst)))
	   (set! THEOREMS (cons (list sname saconst sproof) THEOREMS))))))))

;; Code discarded 2025-06-14
   ...
   ((proof-in-imp-intro-form? proof)
    (let* ((avar (proof-in-imp-intro-form-to-avar proof))
	  ...
	  (let* ((kernel-avars (proof-to-free-avars kernel))
		 ...
		 (invarex-aconst ;all xs(A -> exnc z(z mr A))
	 ...
	 ((member idpc-name '("AndL" "AndR"))
	  (let* ((cterms (idpredconst-to-cterms idpc))
	    ...
	    (andlr-formula-and-concl-to-andlr-elim-mr-proof
	     andlr-formula concl)))
	 ;; commented out 2026-02-07
	 ;; ((member idpc-name '("AndD")) ;added 2026-02-01
	 ;;  (let* ((cterms (idpredconst-to-cterms idpc))
	 ;; 	 (left-cterm (car cterms))
	 ;; 	 (right-cterm (cadr cterms))
	 ;; 	 (left (cterm-to-formula left-cterm))
	 ;; 	 (right (cterm-to-formula right-cterm))
	 ;; 	 (andd-formula (make-andd left right)))
	 ;;    (andd-formula-and-concl-to-andd-elim-mr-proof
	 ;;     andd-formula concl)))
...
(define totalmr-to-eqd-aconst
  (make-aconst
   "TotalMRToEqD"
   'axiom
   (pf "all alpha^1,alpha^2(TotalMR alpha^1 alpha^2 ->
...
(define totalnc-to-totalmr-aconst
  (make-aconst
   "TotalNcToTotalMR"
   'axiom
   (pf "all alpha^(TotalNc alpha^ -> TotalMR alpha^ alpha^)")
   empty-subst))
...
;; TotalVarBoole
(set-goal "all boole TotalBoole boole")
(assume "boole")
(use "TotalVar")
;; Proof finished.
;; (cp)
...
(pp "TotalYprodPairConstr")

;; all alpha1^(
;;  Total alpha1^ -> 
;;  all alpha2^0(Total alpha2^0 -> TotalYprod(alpha1^ pair alpha2^0)))

;; TotalPairTrueDummy
(set-goal "TotalYprod(True pair Dummy)")
(use "TotalYprodPairConstr")
(intro 0)
(intro 0)
;; Proof finished.
;; (cp)
(save "TotalPairTrueDummy")

;; TotalVarYprodBooleUnit
(set-goal "all (boole yprod unit) TotalYprod(boole yprod unit)")
(assume "(boole yprod unit)")
(use "TotalVar")
;; Proof finished.
;; (cp)
(save "TotalVarYprodBooleUnit")
...
;; 	  (theorem-name-to-proof "ExDTotalElim")))))
...
;; (pp (rename-variables
;;      (nt (proof-to-extracted-term
;; 	  (theorem-name-to-proof "ExLTotalElim")))))
...
;; (pp (rename-variables
;;      (nt (proof-to-extracted-term
;; 	  (theorem-name-to-proof "ExRTotalElim")))))
...
;; ExRMRElim
(set-goal "all gamma^((ExRMR alpha gamma
...
;; UnitEqToEqD
(set-goal "all unit,unit0(unit=unit0 -> unit eqd unit0)")
(cases)
(cases)
(assume "Useless")
(intro 0)
...
;; ;; NotExToAllNot
;; (set-goal "(exnc alpha (Pvar alpha)^ alpha -> F) -> 
;; allnc alpha((Pvar alpha)^ alpha -> F)")
;; (assume "NotEx" "alpha" "Px")
;; (use "NotEx")
;; (intro 0 (pt "alpha"))
;; (use "Px")
;; ;; Proof finished.
...
;; AllNotToNotEx
(set-goal "allnc alpha((Pvar alpha)^ alpha -> F) ->
 exnc alpha (Pvar alpha)^ alpha -> F")
(assume "AllNot" "Ex")
(elim "Ex")
(use "AllNot")
;; Proof finished.
...
;; OrcaElim
(set-goal "(((Pvar^0 -> F) -> F) -> Pvar^0)  ->
 ((Pvar^1 -> F) -> (Pvar^2 -> F) -> F) ->
 (Pvar^1 -> Pvar^0) -> (Pvar^2 -> Pvar^0) -> Pvar^0")
(assume "Stab0" "OrcaHyp" "Imp1" "Imp2")
(use "Stab0")
(assume "u1")
(use "OrcaHyp")
;; 5,6
...
;; NegAnd and NegOr might be used as rewrite rules

;; NegAnd
(set-goal
 "all boole1,boole2 negb(boole1 andb boole2)=(negb boole1 orb negb boole2)")
(cases)
(strip)
(use "Truth")
(strip)
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "negb(boole1 andb boole2)" "negb boole1 orb negb boole2")
(save "NegAnd")

;; NegOr
(set-goal
 "all boole1,boole2 negb(boole1 orb boole2)=(negb boole1 andb negb boole2)")
(cases)
(strip)
(use "Truth")
(strip)
...
;; Two general lemmata on indirect proofs for atoms

;; ImpAtomIndirect
(set-goal
 "all boole1,boole2((boole2=False -> boole1=False) -> boole1 -> boole2)")
(cases)
(cases)
(auto)
(cases)
...
;; Proof finished.
;; (cp)
(save "ImpAtomIndirect")

;; Not used:
;; ImpAtomIndirectInv
(set-goal
 "all boole1,boole2((boole1 -> boole2) -> boole2=False -> boole1=False)")
(cases)
(cases)
(auto)
;; Proof finished.
;; (cp)
(save "ImpAtomIndirectInv")

;; 2024-10-04

;; IfTrue
(set-goal "all alpha_1,alpha_2 [if True alpha_1 alpha_2]eqd alpha_1")
(ng #t)
(strip)
(use "InitEqD")
;; Proof finished.
...
;; IfFalse
(set-goal "all alpha_1,alpha_2 [if False alpha_1 alpha_2]eqd alpha_2")
(ng #t)
(strip)
(use "InitEqD")
;; Proof finished.
...
;; TotalApp
(set-goal "all (alpha=>beta)^,alpha^(
 Total(alpha=>beta)^ -> Total alpha^ -> Total((alpha=>beta)^ alpha^))")
(assume "(alpha=>beta)^" "alpha^" "AllH" "TH")
(use "AllH")
(use "TH")
;; Proof finished.
;; (cp)
