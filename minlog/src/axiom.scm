...
(define (aconst-to-formula aconst)
  (let* ((inst-formula (aconst-to-inst-formula aconst))
	 (nc-aconst? (formula-of-nulltype? inst-formula))
	 (name (aconst-to-name aconst))
	 (free
    ...
    (apply (if nc-aconst? mk-all mk-allnc)
	   (append free (list inst-formula)))))
  ...
  (let* ((name (cadr x))
    ...
    (if (and (eq? kind 'axiom)
		...
		'("AndAtomToLeft" "AndAtomToRight" "AtomToImp"
		  ...
		  "AllTotalIntro" "AllTotalIntroSound" ;obsolete
		  "AllTotalElim" "AllTotalElimSound" ;obsolete
		  "AllncTotalIntro" "AllncTotalIntroSound" ;obsolete
		  "AllncTotalElim" "AllncTotalElimSound" ;obsolete
		  "InvarEx" "InvarAll"
		...
		(map
		 ...
		 '("=-to-E-"
		   ...
		   "SE-to-E-"
		   "Total-to-E-")))
	       (or (string<=? "AllTotalIntro" name)
		   (string<=? "AllTotalElim" name)
		   (string<=? "AllTotalNcIntro" name)
		   (string<=? "AllTotalNcElim" name)
		   (string<=? "AllTotalIntroSound" name)
		   (string<=? "AllTotalElimSound" name)
		   (string<=? "AllTotalNcIntroSound" name)
		   (string<=? "AllTotalNcElimSound" name)))))
...
(define (finalg-to-=-to-e-1-aconst finalg)
  (let* ((name (default-var-name finalg))
	 ...
	 (formula-of-=-to-e-1-aconst
	  (mk-all var1 var2 (mk-imp =-fla e-fla)))
...
(define (finalg-to-se-to-e-aconst finalg)
  (let* ((name (default-var-name finalg))
	 ...
	 (formula-of-se-to-e-aconst
	  (mk-all var (mk-imp se-fla e-fla)))
...
(define (finalg-to-total-to-e-aconst finalg)
  (let* ((name (default-var-name finalg))
	 ...
	 (formula-of-total-to-e-aconst
	  (mk-all var (mk-imp total-fla e-fla)))
...
(define (alg-to-total-to-stotal-aconst alg)
  (let* ((name (default-var-name alg))
	 ...
	 (formula-of-total-to-stotal-aconst
	  (mk-all var (mk-imp total-fla stotal-fla)))
...
(define alltotal-intro-aconst
  (let* ((tvar (make-tvar -1 DEFAULT-TVAR-NAME))
	 ...
	 (formula-of-alltotal-intro-aconst (mk-imp all-fla alltotal-fla)))
    (make-aconst "AllTotalIntro"
		 'axiom formula-of-alltotal-intro-aconst empty-subst)))

;; (pp (aconst-to-formula alltotal-intro-aconst))
;; all alpha^(Total alpha^ -> (Pvar alpha)alpha^) -> all alpha (Pvar alpha)alpha
...
(define alltotal-elim-aconst
  (let* ((tvar (make-tvar -1 DEFAULT-TVAR-NAME))
	 ...
	 (all-fla
	  (mk-all var (mk-imp (make-total varterm)
			      (make-predicate-formula pvar varterm))))
	 (formula-of-alltotal-elim-aconst (mk-imp alltotal-fla all-fla)))
    (make-aconst "AllTotalElim"
		 'axiom formula-of-alltotal-elim-aconst empty-subst)))

;; (pp (aconst-to-formula alltotal-elim-aconst))
;; all alpha (Pvar alpha)alpha -> all alpha^(Total alpha^ -> (Pvar alpha)alpha^)
...
(define allnctotal-intro-aconst
  (let* ((tvar (make-tvar -1 DEFAULT-TVAR-NAME))
	 ...
	 (all-impnc-fla
	  (mk-all var (mk-imp (make-totalnc varterm)
			      (make-predicate-formula pvar varterm))))
	 (formula-of-allnctotal-intro-aconst
	  (mk-imp all-impnc-fla allnctotal-fla)))
...
;; all alpha^(TotalNc alpha^ -> (Pvar alpha)alpha^) -> 
...
(define allnctotal-elim-aconst
  (let* ((tvar (make-tvar -1 DEFAULT-TVAR-NAME))
	 ...
	 (formula-of-allnctotal-elim-aconst
	  (mk-imp allnctotal-fla all-impnc-fla)))
...
(define (all-formulas-to-ind-aconst . all-formulas)
  (if (null? all-formulas)
      (myerror "all-formulas-to-ind-aconst" "all-formulas expected"))
  (if (not (apply and-op (map t-deg-one?
			      (map var-to-t-deg
				   (map all-form-to-var all-formulas)))))
      (apply myerror
	     "all-formulas-to-ind-aconst"
	     "total variables expected in all-formulas"
	     all-formulas))
  (if (not (apply and-op (map alg-form?
			      (map var-to-type
				   (map all-form-to-var all-formulas)))))
      (apply myerror
	     "all-formulas-to-ind-aconst"
	     "variables of alg types expected in all-formulas"
	     all-formulas))
  (if (not (apply and-op (map alg-inhabited?
			      (map alg-form-to-name
				   (map var-to-type
					(map all-form-to-var all-formulas))))))
      (apply myerror
	     "all-formulas-to-ind-aconst"
	     "variables over totally inhabited algebras expected in all formulas"
	     all-formulas))
  (if (nested-alg-name?
...
(define (all-formulas-to-uninst-imp-formulas-and-tpsubst . all-formulas)
  (if
   ...
   (let* ((free (apply union (map formula-to-free all-formulas)))
	  ...
	  ;; 	      all-formulas)))
	  ;; (alg-form-test
	  ;;  (if (not (apply and-op (map alg-form? (map var-to-type vars))))
	  ;;      (apply myerror
	  ;; 	      "all-formulas-to-uninst-imp-formulas-and-tpsubst"
	  ;; 	      "alg variables expected in all formulas"
	  ;; 	      all-formulas)))
	  ;; (totally-inhabited-test
	  ;;  (if (not (apply and-op (map alg-inhabited?
	  ;; 			       (map alg-form-to-name
	  ;; 				    (map var-to-type vars)))))
	  ;;      (apply myerror
	  ;; 	      "all-formulas-to-uninst-imp-formulas-and-tpsubst"
	  ;; 	      "totally inhabited variables expected in all formulas"
	  ;; 	      all-formulas)))
...
(define (ex-formula-to-ex-intro-aconst ex-formula)
  (let* ((var (ex-form-to-var ex-formula))
	 ...
	 (pvar (if (nulltype? (cterm-to-formula cterm))
		   (arity-to-new-harrop-pvar arity)
...
(define (ex-formula-and-concl-to-ex-elim-aconst ex-formula concl)
  (let* ((var (ex-form-to-var ex-formula))
	 ...
	 (pvar1 (if (nulltype? (cterm-to-formula cterm1))
		    (arity-to-new-harrop-pvar arity1)
	 ...
	 (pvar2 (if (nulltype? (cterm-to-formula cterm2))
		    (arity-to-new-harrop-pvar arity2)
	 ...
	 (uninst-elim-formulas
	  (if ;IMR case
	   ...
	   (map (lambda (new-var-list uninst-elim-formula-in-imp-form)
	 	  (make-all (car (reverse new-var-list)) ;change only here
...
(define (finalg-to-bisim-aconst finalg)
  ...
  (let* ((name (default-var-name finalg))
	 ...
	 (formula-of-coeqpnc-to-eqd-aconst
	  (mk-all var1 var2 (make-imp coeqpncfla eqdfla))))
...
(define (bisim-cterm-to-formula-of-bisim-aconst bisim-cterm)
  (let* ((vars (cterm-to-vars bisim-cterm))
    ...
    (mk-all x y (make-imp fla (make-eqd (make-term-in-var-form x)
...
(define (theorem-name-to-formula name)
  (aconst-to-formula (theorem-name-to-aconst name)))
...
(define (add-theorem str . opt-proof)
  ...
  (let ((proof (if (null? opt-proof)
     ...
     *the-non-printing-object*
     (let ((formula (unfold-formula (proof-to-formula proof)))
       ...
       (let ((aconst (make-aconst str 'theorem formula empty-subst)))
	 (set! THEOREMS (cons (list str aconst proof) THEOREMS))
	 (if (not (member str (list "Id" "If")))
	     (comment "ok, " str " has been added as a new theorem."))
	 (if (and (final-substring? "Sound" str)
		  (let* ((name (substring str 0 (- (string-length str)
						      (string-length "Sound"))))
...
(define (add-theorem-silent string . opt-proof)
  (if (and (null? opt-proof)
	   (null? PPROOF-STATE))
      (myerror
       "add-theorem-silent"
	 ...
	 (if
	  ...
	  (let* ((name (substring string 0 (- (string-length string)
	    ...
	    (if
	     ...
	     (begin
	       ...
	       (myerror "add-theorem-silent" "formula of theorem" formula
			"should be equal to the totality formula"
			(term-to-totality-formula term)
			"or the unfolded totality formula"
			(term-to-unfolded-totality-formula term))))
	  ...
	  (if (not (or (formula-with-mr-predicates? formula)
	      ...
	      (let* ((pconst-name
		     ...
		     (type (formula-to-et-type formula))
		     (t-deg (term-to-t-deg (proof-to-extracted-term proof))))
		(add-program-constant-silent
		 pconst-name type t-deg 'const 0 (t-deg-one? t-deg))))))))))

(define save-silent add-theorem-silent)
...
;; (add-program-constant "StarOne" (py "alpha@@beta=>alpha"))
;; (add-program-constant "StarTwo" (py "alpha@@beta=>beta"))

(define boole-alltotal-intro-aconst
  (let* ((type (make-alg "boole"))
	 (vartotal (make-var type -1 t-deg-one ""))
	 (var (make-var type -1 t-deg-zero ""))
	 (vartotalterm (make-term-in-var-form vartotal))
	 (varterm (make-term-in-var-form var))
	 (pvar (make-pvar (make-arity type) -1 h-deg-zero n-deg-zero ""))
	 (alltotal-fla
	   (mk-all vartotal (make-predicate-formula pvar vartotalterm)))
	 (all-fla
	  (mk-all var (mk-imp (make-total varterm)
			      (make-predicate-formula pvar varterm))))
	 (fla (mk-imp all-fla alltotal-fla)))
    (make-aconst "AllTotalIntroBoole" 'axiom fla empty-subst)))

;; (pp (aconst-to-formula boole-alltotal-intro-aconst))
;; all boole^(TotalBoole boole^ -> (Pvar boole)boole^) -> 
;; all boole (Pvar boole)boole

(define boole-alltotal-elim-aconst
  (let* ((type (make-alg "boole"))
	 (vartotal (make-var type -1 t-deg-one ""))
	 (var (make-var type -1 t-deg-zero ""))
	 (vartotalterm (make-term-in-var-form vartotal))
	 ...
	 (all-fla
	  (mk-all var (mk-imp (make-total varterm)
			      (make-predicate-formula pvar varterm))))
	 (fla (mk-imp alltotal-fla all-fla)))
    (make-aconst "AllTotalElimBoole" 'axiom fla empty-subst)))

;; (pp (aconst-to-formula boole-alltotal-elim-aconst))
;; all boole (Pvar boole)boole -> 
;; all boole^(TotalBoole boole^ -> (Pvar boole)boole^)

(define boole-allnctotal-intro-aconst
  (let* ((type (make-alg "boole"))
	 (vartotal (make-var type -1 t-deg-one ""))
	 (var (make-var type -1 t-deg-zero ""))
	 (vartotalterm (make-term-in-var-form vartotal))
	 ...
	 (all-fla
	  (mk-all var (mk-imp (make-totalnc varterm)
			      (make-predicate-formula pvar varterm))))
	 (fla (mk-imp all-fla allnctotal-fla)))
    (make-aconst "AllNcTotalIntroBoole" 'axiom fla empty-subst)))

;; (pp (aconst-to-formula boole-allnctotal-intro-aconst))
;; all boole^(TotalBooleNc boole^ -> (Pvar boole)boole^) -> 
;; allnc boole (Pvar boole)boole

(define boole-allnctotal-elim-aconst
  (let* ((type (make-alg "boole"))
	 (vartotal (make-var type -1 t-deg-one ""))
	 (var (make-var type -1 t-deg-zero ""))
	 (vartotalterm (make-term-in-var-form vartotal))
	 ...
	 (all-fla
	  (mk-all var (mk-imp (make-totalnc varterm)
			      (make-predicate-formula pvar varterm))))
	 (fla (mk-imp allnctotal-fla all-fla)))
    (make-aconst "AllNcTotalElimBoole" 'axiom fla empty-subst)))

;; (pp (aconst-to-formula boole-allnctotal-elim-aconst))
;; allnc boole (Pvar boole)boole -> 
;; all boole^(TotalBooleNc boole^ -> (Pvar boole)boole^)

;; End of additions 2025-10-07.
