...
;; 2024-03-15.  Replace use of NatLeAntiSym by simp with
;; NatPlusCancelEqL or NatPlusCancelEqR, in libnseq.scm and elsewhere.

;; 2023-09-05.  If in an inductive definition a syntax error occurs,
;; do (set! COMMENT-FLAG #t) (remove-pvar-name "...").  This should
;; be mentioned in ref.tex

;; 2023-09-05.  (search) does not always check occurrences of the goal
;; in the premises.

;; 2023-09-05.  by-assume should accept multiple arguments, as assume.

;; 2023-09-05.  The suffix f for functions collides with if in case i
;; is used as var-name for nat.  Therefore: use suffix l for lists and
;; s for functions.  This means that the entry "Variable ps replaced
;; by pf" (for nat.scm in log.txt) must be taken back.

;; 2023-09-05.  Simplify [if boole t s] in case (negb boole) to s not
;; only in case (boole -> F) Then one does not need to apply NotToNeg
;; NegToNot.

;; 2023-09-05.  IfOrb and IfOrbEq are the same.  Remove one of them.
