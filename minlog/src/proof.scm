...
;; Commented out 2025-11-20
;; (define (make-proof-in-andd-elim-right-form andd-proof)
;;   (let* ((fla (proof-to-formula andd-proof))
...
;; 	 (avar1 (formula-to-new-avar left))
;; 	 (avar2 (formula-to-new-avar right))
...
;; 			  avar1
;; 			  (make-proof-in-imp-intro-form
;; 			   avar2 (make-proof-in-avar-form avar2))))))))

;; (define (make-proof-in-andd-elim-left-form andd-proof)
;;   (let* ((fla (proof-to-formula andd-proof))
;; 	 (left (if (andd-form? fla)
;; 		   (andd-form-to-left fla)
;; 		   (myerror "make-proof-in-andd-elim-left-form"
...
;; 			  avar1
;; 			  (make-proof-in-imp-intro-form
;; 			   avar2 (make-proof-in-avar-form avar1))))))))

;; Revived 2025-11-20
;; Added 2025-11-12 in commented-out form
	 ...
	 (renaming-psubst
	  (map (lambda (pvar)
		 (list pvar
			...
			((if (= (pvar-to-h-deg pvar) h-deg-zero)
			     arity-to-new-general-pvar
	 ...
	 (tsubst-param-pvars
	  (map (lambda (orig-param-pvar)
	 	 (let* ((arity (predicate-to-arity orig-param-pvar))
	 	   ...
	 	   ((if (= (pvar-to-h-deg orig-param-pvar) h-deg-zero)
			arity-to-new-general-pvar
			arity-to-new-harrop-pvar)
		    subst-arity)))
	 		...
	 		(subst-arity
	 		 (apply make-arity
	 			(map (lambda (type)
	 			       (type-substitute type tsubst))
	 			     (arity-to-types arity)))))
	 	   ((if (= (pvar-to-h-deg orig-simidpc-pvar) h-deg-zero)
			arity-to-new-general-pvar
			arity-to-new-harrop-pvar)
		    subst-arity)))
	 	   ;; (arity-to-new-general-pvar subst-arity)))
...
;; A(Ps) -> (all xs(P_i xs -> Q_i xs))_i -> A(Qs).
...
(define (formula-and-psubsts-to-mon-proof formula psubst1 psubst2)
  (let* ((pvars (map car psubst1)) ;Xs
	 ...
	 (sub-formulas ;allnc xs(P xs -> Q xs)
	  (map (lambda (vars imp)
		 (rename-variables
		  (apply mk-all (append vars (list imp)))))
	       var-lists imps))
...
(define (check-proof display-flag . opt-proof-or-thm-name-and-ignore-deco-flag)
  (let* ((proof-and-ignore-deco-flag
	  (cond
	    ...
	    (let ((first (car opt-proof-or-thm-name-and-ignore-deco-flag)))
	      (cond ((proof-form? first) (list first #f))
		    ...
		    (else (myerror "check-proof"
				   "proof, string or boolean expected"
				   first)))))
	   ((= 2 (length opt-proof-or-thm-name-and-ignore-deco-flag))
	    (let ((first (car opt-proof-or-thm-name-and-ignore-deco-flag))
	      ...
	      (cond
	       ((and (proof-form? first) (boolean? second))
		(list first second))
	       ((and (string? first) (boolean? second))
		(list (theorem-name-to-proof first) second))
	       (else (myerror "check-proof"
			      "(proof or string) and boolean expected"
			      first second)))))
	   (else (myerror "check-proof"
	 ...
	 (free-avars (proof-to-free-avars proof))
	 (aconsts (proof-to-aconsts-with-repetitions proof))
   ...
   ((proof-in-all-elim-form? proof)
	...
	(if (and (t-deg-one? (var-to-t-deg (all-form-to-var op-fla)))
	    ...
	    (myerror "degrees of totality do not fit for variable"
		     (all-form-to-var op-fla) "and term" arg))
	;; (if (and (t-deg-one? (var-to-t-deg (all-form-to-var op-fla)))
	;; 	 (let* ((free (term-to-free arg))
	;; 		(types (map var-to-type free))
	;; 		(lst (map totally-inhabited? types)))
	;; 	   (member #f lst)))
	;;     (myerror "totally inhabited types expected in the free variables of"
	;; 	     arg))
      ...
      ((imp) ;A -> B -> ... -> I ts
       (let* ((prem (imp-form-to-premise formula)) ;A
	      ...
	      (prev-idpc-proof ;of B -> ... -> I ts from u:F
		   ...
		   (myerror "idpc-proof-and-falsity-avar-to-ef-idpc-proof"
			    "initial clause of" idpc-name
                            "has a c.r. premise" prem)
  ...
  (cond
   ...
   ((quant-form? expr) ;to be done before inductively defined predicates
    (let* ((quant (quant-form-to-quant expr))
	   ...
	   (totalnc-fla
	    (term-to-totalnc-formula (make-term-in-var-form partial-var)))
	   (unfolded-kernel
	    (apply unfold-total-variables kernel var vars)))
      (cond
       ((all-form? expr)
	(if (t-deg-one? (var-to-t-deg var))
	    (make-all partial-var (make-imp totality-fla unfolded-kernel))
	    (make-all var (apply unfold-total-variables kernel vars))))
       ((allnc-form? expr)
	(if (t-deg-one? (var-to-t-deg var))
	    (make-all partial-var (make-imp totalnc-fla unfolded-kernel))
	    (make-all var (apply unfold-total-variables kernel vars))))
       ((exdt-form? expr)
	...
	(if (t-deg-one? (var-to-t-deg var))
		...
		(make-exr
		 partial-var (make-andl totality-fla unfolded-kernel))
		(myerror "unfold-total-variables" "n.c. formula expected"
			 kernel))
	    (myerror "unfold-total-variables" "total variable expected in"
		     expr)))
       ((exrt-form? expr)
	(if (t-deg-one? (var-to-t-deg var))
	    (if (formula-of-nulltype? kernel)
		(myerror "unfold-total-variables" "c.r. formula expected"
			 kernel)
		(make-exr partial-var (make-andr totalnc-fla unfolded-kernel)))
	    (myerror "unfold-total-variables" "total variable expected in"
		     expr)))
       ((exnct-form? expr)
	(if (t-deg-one? (var-to-t-deg var))
	    (if (formula-of-nulltype? kernel)
		(make-exnc partial-var (make-andnc totalnc-fla unfolded-kernel))
		(myerror "unfold-total-variables" "n.c. formula expected"
			 kernel))
	    (myerror "unfold-total-variables" "total variable expected in"
		     expr)))
       ((ex-form? expr)
	(make-ex var (apply unfold-total-variables  vars)))
...
;; Tests
;; (pp (unfold-total-variables (pf "all boole (Pvar boole)boole")))
;; all boole^(TotalBoole boole^ -> (Pvar boole)boole^)
;; (pp (unfold-total-variables (pf "allnc boole (Pvar boole)boole")))
;; all boole^(TotalBooleNc boole^ -> (Pvar boole)boole^)
;; (pp (unfold-total-variables (pf "exd boole (Pvar boole)boole")))
;; exr boole^(TotalBoole boole^ andd (Pvar boole)boole^)
;; (pp (unfold-total-variables (pf "exl boole (Pvar boole)^ boole")))
;; exr boole^(TotalBoole boole^ andl (Pvar boole)^ boole^)
;; (pp (unfold-total-variables (pf "exr boole (Pvar boole)boole")))
;; exr boole^(TotalBooleNc boole^ andr (Pvar boole)boole^)
;; (pp (unfold-total-variables (pf "exnc boole (Pvar boole)^ boole")))
;; exnc boole^(TotalBooleNc boole^ andnc (Pvar boole)^ boole^)
  ...
  (cond
   ...
   ((quant-form? expr) ;to be done before inductively defined predicates
    (let* ((quant (quant-form-to-quant expr))
	   ...
	   (totalnc-fla
	    (term-to-totalnc-formula (make-term-in-var-form var))))
      (cond
       ((and (all-allnc-form? expr)
	     (imp-form? kernel)
	     (t-deg-zero? (var-to-t-deg var))
	     (formula=? (imp-form-to-premise kernel) totality-fla))
	(make-all total-var (apply fold-total-variables
				   (imp-form-to-conclusion kernel) var vars)))
       ((and (all-allnc-form? expr)
	     (imp-form? kernel)
	     (t-deg-zero? (var-to-t-deg var))
	     (formula=? (imp-form-to-premise kernel) totalnc-fla))
	(make-allnc total-var (apply fold-total-variables
				     (imp-form-to-conclusion kernel) var vars)))
       ((all-form? expr)
	(make-all var (apply fold-total-variables kernel vars)))
       ((allnc-form? expr)
	(make-allnc var (apply fold-total-variables kernel vars)))
       ((and (exr-form? expr)
	     (t-deg-zero? (var-to-t-deg var)))
	(cond
	 ((and (andd-form? kernel)
	       (formula=? (andd-form-to-left kernel) totality-fla))
	  (make-exdt total-var (apply fold-total-variables
				      (andd-form-to-right kernel) var vars)))
	 ((and (andl-form? kernel)
	       (formula=? (andl-form-to-left kernel) totality-fla))
	  (make-exlt total-var (apply fold-total-variables
				      (andl-form-to-left kernel) var vars)))
	 ((and (andr-form? kernel)
	       (formula=? (andr-form-to-left kernel) totalnc-fla))
	  (make-exrt total-var (apply fold-total-variables
				      (andr-form-to-right kernel) var vars)))
	 (else (make-exrt var (apply fold-total-variables kernel vars)))))
       ((and (exnc-form? expr)
	     (t-deg-zero? (var-to-t-deg var))
	     (andnc-form? kernel)
	     (formula=? (andnc-form-to-left kernel) totalnc-fla))
	(make-exnct total-var (apply fold-total-variables
				     (andnc-form-to-right kernel) var vars)))
       ((and (exrt-form? expr)
	     (t-deg-one? (var-to-t-deg var)))
	(make-exrt var (apply fold-total-variables kernel vars)))
       ((ex-form? expr)
	(make-ex var (apply fold-total-variables kernel vars)))
       ((or (exca-form? expr)
	    (excl-form? expr)
	    (excu-form? expr))
	(myerror "fold-total-variables" "quantifier" quant
		 "should be unfolded"))
       (else (myerror "fold-total-variables" "quantifier expected" quant)))))
   ((predicate-form? expr) ;not an inductively defined bicon or quant
    (let ((pred (predicate-form-to-predicate expr))
	  (args (predicate-form-to-args expr)))
      (cond
...
;; Tests
;; (pp (fold-total-variables
;;      (pf "all boole^(TotalBoole boole^ -> (Pvar boole)boole^)")))
;; all boole (Pvar boole)boole
;; (pp (fold-total-variables
;;      (pf "all boole^(TotalBooleNc boole^ -> (Pvar boole)boole^)")))
;; allnc boole (Pvar boole)boole
;; (pp (fold-total-variables
;;      (pf "exr boole^(TotalBoole boole^ andd (Pvar boole)boole^)")))
;; exd boole (Pvar boole)boole
;; (pp (fold-total-variables
;;      (pf "exr boole^(TotalBoole boole^ andl (Pvar boole)^ boole^)")))
;; exl boole (Pvar boole)^ boole
;; (pp (fold-total-variables
;;      (pf "exr boole^(TotalBooleNc boole^ andr (Pvar boole)boole^)")))
;; exr boole (Pvar boole)boole
;; (pp (fold-total-variables
;;      (pf "exnc boole^(TotalBooleNc boole^ andnc (Pvar boole)^ boole^)")))
;; exnc boole (Pvar boole)^ boole
