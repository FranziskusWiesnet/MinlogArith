...
(define (make-term-in-if-form test alts . rest) ;rest empty or all-formula
  (if
   ...
   (let* ((alg (term-to-type test))
	  ...
	  (coerce-ops (map types-to-embedding
...
;; Added 2026-02-07:
(define (term-in-pair-form-wrt-yprod? term)
  (let ((op (term-in-app-form-to-final-op term)))
    (and (term-in-const-form? op)
	 (eq? "PairConstr" (const-to-name (term-in-const-form-to-const op))))))

(define (term-to-term-in-pair-form-wrt-yprod term)
  (let* ((type (term-to-type term))
	 (left-type (if (yprod-form? type)
			(yprod-form-to-left-type type)
			(myerror "term-to-term-in-pair-form-wrt-yprod"
				 "type in yprod form expected" type)))
	 (right-type (yprod-form-to-right-type type))
	 (cLft-pconst (cadr (assoc "cLft" PROGRAM-CONSTANTS)))
	 (cRht-pconst (cadr (assoc "cRht" PROGRAM-CONSTANTS)))
	 (tsubst (make-substitution
		  (list (py "beta1") (py "beta2"))
		  (list left-type right-type)))
	 (subst-cLft-pconst (const-substitute cLft-pconst tsubst #t))
	 (subst-cRht-pconst (const-substitute cRht-pconst tsubst #t))
	 (left-app-term
	   (make-term-in-app-form
	    (make-term-in-const-form subst-cLft-pconst) term))
	 (right-app-term
	   (make-term-in-app-form
	    (make-term-in-const-form subst-cRht-pconst) term)))
    (make-term-in-pair-form-wrt-yprod left-app-term right-app-term)))
...
(define (term-to-t-deg-aux term bound-vars)
  (case (tag term)
    ...
    ((term-in-const-form)
     (if (string=? "Inhab" (const-to-name (term-in-const-form-to-const term)))
	 (let* ((const (term-in-const-form-to-const term))
		(type (const-to-type const))
		(val-type (arrow-form-to-final-val-type type)))
	   (if (and (= 0 (type-to-level val-type))
		    (null? (type-to-tvars val-type))
		    (alg-form? val-type)
		    (alg-and-excluded-simalg-names-to-inhab-or-f val-type))
	       t-deg-one
	       t-deg-zero))
	 (const-to-t-deg (term-in-const-form-to-const term))))
    ((term-in-abst-form)
     (let ((var (term-in-abst-form-to-var term))
	   (kernel (term-in-abst-form-to-kernel term)))
  ...
  (define (term-to-consts-info term)
    (cond
	 ...
	 ((string=? name "Destr")
         ...
         ((or (string=? name "GRecGuard") (string=? name "GRec"))
          (let* ((algs (arrow-form-to-arg-types
                        (arrow-form-to-arg-type type))))
            (cons (list kind "GRecGuard" algs '()) prevs)))
         (else
  ...
  (let* ((names (map car opt-name-arity-alist))
	...
	(cond
	 ...
	 ((or (string=? name "IntPos") (string=? name "PosToNat"))
	  (if (= l 1) (car prevs)
		...
		((scheme) (string->symbol name))
		((haskell) "id"))))
         ((and (equal? lang 'haskell) (or (string=? name "cNatPos")
                                          (string=? name "NatToPos")))
           (if (= l 1)
               (string-append "(if ("
                              (car prevs)
                              ") <= 1 then 1 else ("
	 ...
	 ((string=? name "CoRec")
	  (let* ((uninst-alg-or-arrow-types 
	    ...
	    (non-null-list-to-app-expr lang (cons corec-symbol prevs))))
         ((string=? name "GRec")
          ;; Translate the corresponding GRecGuard instead, by normalising term
          (apply term-to-external-expr (nt term) lang module-name opt-name-arity-alist))
	...
	(cond
	 ((and (alg-form? type) (string=? (alg-form-to-name type) "boole"))
          (let* ((test-expr (apply term-to-external-expr
		     ...
		     (list (list 'odd? test-expr)
			   (remove-vacuous-beta-redex
			    (list (caddr alt-exprs)
				  (list 'quotient test-expr 2))))))
	      ((haskell)
               (let ((test-expr-halved (string-append "(" test-expr " `quot` 2)")))
	         (string-append
	          "(if (1 == " test-expr ") then " (car alt-exprs)
	          " else (if (even " test-expr ") then (" (cadr alt-exprs) test-expr-halved ") else (" (caddr alt-exprs) test-expr-halved ")))"))))))
	 ...
	 ((alg-form? type)
	  (case lang
	    ...
	    ((haskell)
             (let* ((test-expr (apply term-to-external-expr
			              (term-to-beta-eta-nf test) lang
				      module-name
			              opt-name-arity-alist))
	            (constr (alg-name-to-typed-constr-names
			     (alg-form-to-name type)))
	            (foreach-constr
		     (lambda (typed-constr-name alt)
		       (let* ((constr-name
			       (typed-constr-name-to-name typed-constr-name))
			      (constr-name-expr
			       (apply term-to-haskell-expr
				      (make-term-in-const-form
				       (constr-name-to-constr constr-name))
			      ...
			      (patargs
			       (map make-term-in-var-form
                                    (term-in-abst-form-to-vars alt-expanded)))
			      (lhs (non-null-list-to-app-expr
				     'haskell
				     (cons constr-name-expr
				           (map (lambda (x)
					          (apply term-to-haskell-expr x
						         (list module-name)))
					        patargs))))
                              (rhs-term (term-in-abst-form-to-final-kernel
					 alt-expanded))
		    ...
		    ;; (foreach-constr
		     ...
		     ;; 		      (list module-name)))
                     ;;          (alt-expanded
                     ;;           (cond
                     ;;            ((term-in-abst-form? alt) alt)
                     ;;            ((term-in-const-form? alt)
                     ;;             (let* ((arg-types (list-head
                     ;;                                (arrow-form-to-arg-types
                     ;;                                 (term-to-type alt))
                     ;;                                (length
                     ;;                                 (arrow-form-to-arg-types
		     ...
		     ;; 				         (list module-name)))
		     ;; 			        patargs))))
                     ;;          (rhs-term (term-in-abst-form-to-final-kernel alt-expanded))
		     ;; 	      (rhs (apply term-to-haskell-expr rhs-term
                     ;;                      (list module-name))))
		     ;;     (string-append lhs " -> " rhs ))))
...
(define (type-to-haskell-inhab type module-name)
  (cond
   ;; Since we translate nat and pos to Integer even if int.scm was
   ;; not loaded, we need to deal with int as a special case here
   ((and (alg-form? type) (string=? (alg-form-to-name type) "int"))
   ...
   ((alg-form? type)
    (let* ((name (alg-form-to-name type)))
      (cond ((or (string=? name "nat") (string=? name "pos"))
             (adjoin (make-alg "int") inhab-info))
            ((string=? name "ysumu")
...
(define (rec-op-and-args-to-if-term op args)
  ...
  (let* ((rec-const (term-in-const-form-to-const op))
	 ...
	 (var-lists-list ;list of lists of one or two element lists
	  (map one-two-var-lists dup-indicators arg-vars-list))
	 (argtype-vars-alists
...
(define (nbe-normalize-term term)
  (let ((init (nbe-normalize-term-without-eta
	       (normalize-term-pi-with-rec-to-if
		(term-to-term-with-eta-expanded-if-terms-and-unfolded-simrecs
		 term)))))
    (do ((t init (nbe-normalize-term-without-eta
		  (normalize-term-pi-with-rec-to-if t))))
