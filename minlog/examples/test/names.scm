...
(add-var-name "xl" "yl" "zl" (py "list alpha"))
...
(add-ids (list (list "Even" (make-arity (py "nat")) "nat"))
	 '("Even 0" "InitEven")
	 '("all n^(Even n^ -> Even(n^ +2))" "GenEven"))

(add-ids (list (list "Ev" (make-arity (py "nat")) "algEv")
	       (list "Od" (make-arity (py "nat")) "algOd"))
	 '("Ev 0" "InitEv")
	 '("all n^(Od n^ -> Ev(n^ +1))" "GenEv")
	 '("all n^(Ev n^ -> Od(n^ +1))" "GenOd"))
...
(add-ids (list (list "TrCl" (make-arity (py "alpha") (py "alpha")) "lnat"))
	 '("all x^,y^(R x^ y^ -> TrCl x^ y^)" "InitTrCl")
	 '("all x^,y^,z^(R x^ y^ -> TrCl y^ z^ -> TrCl x^ z^)" "GenTrCl"))

;; The n.c. transitive closure of a relation.

(add-ids
 (list (list "TrClNc" (make-arity (py "alpha") (py "alpha")) "nat"))
 '("all x^,y^(R^ x^ y^ -> TrClNc x^ y^)" "InitTrClNc")
 '("all x^,y^,z^(R^ x^ y^ -> TrClNc y^ z^ -> TrClNc x^ z^)" "GenTrClNc"))

;; The reflexive transitive closure of a relation.

(add-ids (list (list "RTrCl" (make-arity (py "alpha") (py "alpha")) "list"))
	 '("all x^ RTrCl x^ x^" "InitRTrCl")
	 '("all x^,y^,z^(R x^ y^ -> RTrCl y^ z^ -> RTrCl x^ z^)" "GenRTrCl"))
...
;; may use a total variable x instead of x^, and have xl rather than x.
