...
;; TrCl	with content of type lnat
;; 	InitTrCl:	all x^,y^(R x^ y^ -> (TrCl (cterm (x^0,x^1) R x^0 x^1))x^ y^)
;; 	GenTrCl:	all x^,y^,z^(
;;  R x^ y^ -> 
...
;; all x^,y^(R^ x^ y^ -> R775 x^ y^)
;; all x^,y^,z^(R^ x^ y^ -> R775 y^ z^ -> R775 x^ z^)
...
;; all x^,y^(R^ x^ y^ -> R775 x^ y^)
;; all x^,y^,z^(R^ x^ y^ -> R775 y^ z^ -> R775 x^ z^)
;; > all n^,n^0(exl l n^ +l=n^0 -> (TrCl (cterm (n^1,n^2) exl l n^1+l=n^2))n^ n^0)
...
;; all x^,y^,alpha1702^(
;;  (Pvar alpha alpha alpha1702)^780 x^ y^ alpha1702^ -> 
;;  (Pvar alpha alpha lnat alpha1702)^779 x^ y^((LnatZero alpha1702)alpha1702^))
;; all x^,y^,z^,alpha1702^(
;;  (Pvar alpha alpha alpha1702)^780 x^ y^ alpha1702^ -> 
;;  all (lnat alpha1702)^0(
;;   (Pvar alpha alpha lnat alpha1702)^779 y^ z^(lnat alpha1702)^0 -> 
;;   (Pvar alpha alpha lnat alpha1702)^779 x^ z^
;;   ((LnatSucc alpha1702)alpha1702^(lnat alpha1702)^0)))
...
;; 	EqPInfltlistInfLTcons:	all t^,t^0(
;;  EqPInfltree t^ t^0 -> 
;;  all ts^,ts^0(
...
;; 	EqPInfltreeInfLLeaf:	all x^,x^0(
;;  EqP x^ x^0 -> EqPInfltree((InfLLeaf alpha)x^)((InfLLeaf alpha)x^0))
;; 	EqPInfltreeInfLBranch:	all ts^,ts^0(
;;  EqPInfltlist ts^ ts^0 -> 
;;  EqPInfltree((InfLBranch alpha)ts^)((InfLBranch alpha)ts^0))
;; 	EqPInfltreeInfLLim:	all (nat=>infltree alpha)^,(nat=>infltree alpha)^0(
;;  all n^,n^0(
;;   EqPNat n^ n^0 -> 
...
(pp (term-to-totality-formula (pt "xl^")))
;; TotalList xl^

(pp (term-to-totality-formula (pt "nl^")))
;; TotalList nl^
...
;; all n^(CoEven n^ -> n^ eqd 0 orr exr n^0(CoEven n^0 andl n^ eqd n^0+2))
...
;; 	GenEvenMR:	all n^,n^0(EvenMR n^ n^0 -> EvenMR(n^ +2)(Succ n^0))
