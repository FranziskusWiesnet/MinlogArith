;; 2025-11-13.  examples/analysis/graycode.scm
...
;; In term-to-pure-ext-formula term-to-pure-extnc-formula opt-cotype
;; removed.
...
;; ;; In term-to-pure-ext-formula term-to-pure-extnc-formula opt-type
;; ;; reintroduced.  It was called opt-cotype, but no cotype is involved.

;; ;; (define (term-to-pure-ext-formula term . opt-type)
;; ;;   (if (pair? opt-type)
;; ;;       (terms-to-pure-eqp-formula term term (car opt-type))
;; ;;       (terms-to-pure-eqp-formula term term)))

;; ;; (define (term-to-pure-extnc-formula term . opt-type)
;; ;;   (if (pair? opt-type)
;; ;;       (terms-to-pure-eqpnc-formula term term (car opt-type))
;; ;;       (terms-to-pure-eqpnc-formula term term)))
...
;; AgCoRecLR
(set-goal "all gu^,u^,boole^,u^1,gv^(
     gu^ u^ eqd
     (InL (boole yprod(ag ysum gamma1)) (ah ysum gamma2))
...
;; (inst-with-to
;;  "Assertion"
;;  (pt "gu^1") (pt "gu^2") "AInst1")
;; (drop "Assertion")
;; (inst-with-to "AInst1" "gu1=gu2" "AInst2")
...
;;  (pt "gv^1") (pt "gv^2") "gv1=gv2" "AInst") ;error
...
(inst-with-to "Conj1" 'right 'left "PsdMRdclftbgsu")
...
;;    [case bgh0
;;      (InL bg -> 
;;      InL([case bg (b pair ag -> b)]pair InL[case bg (b pair ag -> ag)]))
;;      (InR ah -> InR(InL ah))])
;;  ([bgh0]
;;    [case bgh0
;;      (InL bg -> 
...
;; allnc d,x,boole^(
;;  PsdMR d boole^ -> 
;;  all ag^0(CoGMR x ag^0 -> CoHMR((1#2)*(x+1)*d)(cCoHLr boole^ ag^0)))
