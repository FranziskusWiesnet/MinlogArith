...
(pp (nf (theorem-name-to-formula "ApproxSplitBooleSound")))
...
;;  (cApproxSplit x x0 x1 p))

(pp "cApproxSplitBoole0CompRule")
;; cApproxSplitBoole eqd cApproxSplit

;; (cp "ApproxSplitBooleSound")

(deanimate "ApproxSplitBoole")

;; We first prove an auxiliary lemma IVTAux, for the construction step.

;; We define a (formally inductive) correctness predicate:
...
(pp (theorem-name-to-formula "IVTAux"))
...
;; [f,p,p0]
;;  (cDC rat yprod rat)(f doml pair f domr)([n]cIVTAux f p(NatToPos(p0+n)))
...
(pp (theorem-name-to-formula "IVTcds"))
...
(simp "RatExpSucc")
(simp "RatTimesAssoc")
(simp (pf "(2#3)*(2#3)**n=(2#3)**n*(2#3)"))
(use "Truth")
(use-with "RatTimesComm" (pt "2#3") (pt "(2#3)**n"))
;; Proof finished.
;; (cp)
(save "IVTDiff")
...
(pp (theorem-name-to-formula "TwoThirdExpNatZeroSeq"))
;; all p exl n (2#3)**n<=(1#2**p)

(pp "cTwoThirdExpNatZeroSeq0CompRule")
...
(pp (theorem-name-to-formula "IVTFinal"))
...
;; This needs RealApprox (in rea.scm)
...
(pp (theorem-name-to-formula "IVTApproxSound"))
