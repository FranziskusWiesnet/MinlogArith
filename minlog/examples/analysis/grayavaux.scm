;; 2025-11-21.  examples/analysis/grayavaux.scm.
...
;; usage of JKOne,JKTwo we could employ IntToBoole, BooleToInt, PsdInj and
;; invariance axioms.  However, in the present simple case the latter
;; are easily provable.

;; CoGAvcSatCoIClAuxJ
(set-goal "allnc d,e,i(Psd d -> Psd e -> Sdtwo i -> Sdtwo(JKOne(d+e+i*2)))")
...
(use "SdtwoInjElim"
 (pt "IntToSdtwo(JKOne(BooleToInt boole1+BooleToInt boole2+SdtwoToInt t*2))"))
(simp (pf "JKOne(BooleToInt boole1+BooleToInt boole2+SdtwoToInt t*2)=JKOne(d+e+i*2)"))
...
;; CoGAvcSatCoIClAuxK
(set-goal "allnc d,e,i(Psd d -> Psd e -> Sdtwo i -> Sd(JKTwo(d+e+i*2)))")
(assume "d" "e" "i" "Psdd" "Psde" "Sdtwoi")
(assert "exl boole1 PsdInj d boole1")
(use "PsdInjIntro")
(use "Psdd")
...
(by-assume "ExHyp3" "t" "tProp")
(use "SdInjElim"
     (pt "IntToSd(JKTwo(BooleToInt boole1+BooleToInt boole2+SdtwoToInt t*2))"))
(simp (pf "JKTwo(BooleToInt boole1+BooleToInt boole2+SdtwoToInt t*2)=JKTwo(d+e+i*2)"))
(use "SdInjIntToSd")
...
;; [boole,boole0,t]
;;  IntToSd(JKTwo(BooleToInt boole+BooleToInt boole0+SdtwoToInt t*2))

;; SdtwoPsdToSdtwoJ
(set-goal "allnc i,d(Sdtwo i -> Psd d -> Sdtwo(JKOne(d+i*2)))")
(assume "i" "d" "Sdtwoi")
(elim "Sdtwoi")
;; 3-7
(assume "Psdd")
...
;; SdtwoToSdtwoJ
(set-goal "allnc i(Sdtwo i -> Sdtwo(JKOne(i*2)))")
(assume "i" "Sdtwoi")
(elim "Sdtwoi")
;; 3-7
(ng)
...
;; SdtwoPsdToSdK
(set-goal "allnc i,d(Sdtwo i -> Psd d -> Sd(JKTwo(d+i*2)))")
(assume "i" "d" "Sdtwoi")
(elim "Sdtwoi")
;; 3-7
(assume "Psdd")
...
;; SdtwoToSdK
(set-goal "allnc i(Sdtwo i -> Sd(JKTwo(i*2)))")
(assume "i" "Sdtwoi")
(elim "Sdtwoi")
;; 3-7
(ng)
...
(set-goal "all x,y,d,e,i(Real x -> Real y ->
 ...
 (1#2)*((1#4)*(x+y+JKOne(d+e+i*2))+JKTwo(d+e+i*2)))")
(cases)
(assume "as" "M")
(cases)
...
(simp "EqHyp1")
(drop "EqHyp1")
(assert "(1#8)*as n+(1#8)*bs n+(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2)==
         (1#8)*as n+((1#8)*bs n+(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2))")
 (ng)
 (use "Truth")
(assume "EqvHyp2")
...
(use "RatPlusCompat")
(use "Truth")
;; ?^70:(d#8)+((e#8)+(i#4))==(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2)
(assert "(i#4)==(i*2#8)")
 (ng)
 (simp "<-" "IntTimesAssoc")
 (use "Truth")
...
(simprat "EqvHyp4")
(drop "EqvHyp4")
(assert "(JKTwo(d+e+i*2)#2)==(JKTwo(d+e+i*2)*4#8)")
 (ng)
 (simp "<-" "IntTimesAssoc")
 (use "Truth")
(assume "EqvHyp5")
(simprat "EqvHyp5")
(drop "EqvHyp5")
;; ?^84:(d#8)+((e#8)+(i*2#8))==(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)*4#8)
(simprat "<-" "RatEqvConstrPlus")
(simprat "<-" "RatEqvConstrPlus")
(simprat "<-" "RatEqvConstrPlus")
(assert "all k k=JKOne k+JKTwo k*4")
 (assume "k")
 (simp "IntPlusComm")
 (use "KJProp")
(assume "JKProp")
...
;;   {e}  {y1}  ey1Prop:Psd e andd CoG y1 andl y===(1#2)*(y1+IntN 1)* ~e
;; -----------------------------------------------------------------------------
;; ?^47:(1#4)*(x+y+i)===(1#2)*((1#4)*(x1* ~d+y1* ~e+JKOne(d+e+i*2))+JKTwo(d+e+i*2))
...
;;      (1#2)*((1#4)*(x1* ~d+y1* ~e+JKOne(d+e+i*2))+JKTwo(d+e+i*2))
...
(intro 0 (pt "JKOne(d+i*2)"))
(intro 0 (pt "JKTwo(d+i*2)"))
(intro 0 (pt "x1* ~d"))
(intro 0 (pt "y1"))
(split)
;; ?_86:Sdtwo(JKOne(d+i*2))
...
;;   {y1}  y1Prop:CoH y1 andl y===(1#2)*y1
;; -----------------------------------------------------------------------------
;; ?^100:(1#4)*(x+y+i)===(1#2)*((1#4)*(x1* ~d+y1+JKOne(d+i*2))+JKTwo(d+i*2))
...
;;       (1#2)*((1#4)*(x1* ~d+y1+JKOne(d+i*2))+JKTwo(d+i*2))
...
(by-assume "CoGClosureInstyLeft" "e" "eProp")
(by-assume "eProp" "y1" "ey1Prop")
(intro 0 (pt "JKOne(e+i*2)"))
(intro 0 (pt "JKTwo(e+i*2)"))
(intro 0 (pt "x1"))
(intro 0 (pt "y1* ~e"))
(split)
;; ?_151:Sdtwo(JKOne(e+i*2))
(use "SdtwoPsdToSdtwoJ")
(use "Sdtwoi")
(use "ey1Prop")
(split)
...
;;   {e}  {y1}  ey1Prop:Psd e andd CoG y1 andl y===(1#2)*(y1+IntN 1)* ~e
;; -----------------------------------------------------------------------------
;; ?^163:(1#4)*(x+y+i)===(1#2)*((1#4)*(x1+y1* ~e+JKOne(e+i*2))+JKTwo(e+i*2))
...
;;       (1#2)*((1#4)*(x1+y1* ~e+JKOne(e+i*2))+JKTwo(e+i*2))
...
(assume  "CoGClosureInstyRight")
(by-assume  "CoGClosureInstyRight" "y1" "y1Prop")
(intro 0 (pt "JKOne(i*2)"))
(intro 0 (pt "JKTwo(i*2)"))
(intro 0 (pt "x1"))
(intro 0 (pt "y1"))
(split)
;; ?_204:Sdtwo(JKOne(i*2))
(use "SdtwoToSdtwoJ")
(use "Sdtwoi")
(split)
;; ?_207:Sd(JKTwo(i*2))
...
;;   {y1}  y1Prop:CoH y1 andl y===(1#2)*y1
;; -----------------------------------------------------------------------------
;; ?^214:(1#4)*(x+y+i)===(1#2)*((1#4)*(x1+y1+JKOne(i*2))+JKTwo(i*2))
...
;;   {y1}  y1Prop:CoH y1 andl y===(1#2)*y1
;; -----------------------------------------------------------------------------
;; 217:(1#4)*((1#2)*(x1+0)+(1#2)*(y1+0)+i)===(1#2)*((1#4)*(x1+y1+JKOne(i*2))+JKTwo(i*2))
...
;;    (InL bg -> [case (DesYprod ag0)
;;      (InL bg0 -> IntToSdtwo(JKOne(BooleToInt clft bg+
;;                               BooleToInt clft bg0+
;;                               SdtwoToInt t*2))pair 
;;                  IntToSd(JKTwo(BooleToInt clft bg+
;;                            BooleToInt clft bg0+
;;                            SdtwoToInt t*2))pair 
;;      cCoGPsdTimes crht bg(cPsdUMinus clft bg)pair 
;;      cCoGPsdTimes crht bg0(cPsdUMinus clft bg0))
...
;;      (InR ah0 -> cSdtwoToSdtwoJK t pair 
;;                  cSdtwoToSdK t pair
;;                  cCoHToCoG ah pair
;;                  cCoHToCoG ah0)])]
...
'("cCoHClause" "cCoGClause" "cIntHalfDsdToSd" "cIntTimesSdtwoPsdToSdtwo"
  "cPsdToBooleToIntValue" "cPsdInjElim" "cPsdInjIntro"
  "cSdtwoToSdtwoToIntValue" "cSdtwoInjElim" "cSdtwoInjIntro"
  "cCoEqPSdtwoClause" "cCoTotalSdtwoClause" "cSdInjElim"
  "cSdInjIntro" "cCoEqPSdClause" "cCoTotalSdClause")
