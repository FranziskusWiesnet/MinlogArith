;; 2025-07-28.  examples/analysis/sdavaux.scm
...
(add-var-name "uv" (py "ai yprod ai"))
(add-var-name "suv" (py "sd yprod ai yprod ai"))
...
;; [u,u0]
;;  cIntPlusSdToSdtwo[if (cCoIClosure u) ([s,u1]s)]
;;  [if (cCoIClosure u0) ([s,u1]s)]pair
;;  [if (cCoIClosure u) ([s,u1]u1)]pair[if (cCoIClosure u0) ([s,u1]u1)]

(add-sound "CoIAvToAvc")

(pp (nf (theorem-name-to-formula "CoIAvToAvcSound")))
...
;;       tuv^0))
;;     tuv^))
;;   (cIntPlusSdToSdtwo[if (cCoIClosure u^) ([s1,u2]s1)]
;;    [if (cCoIClosure u^0) ([s1,u2]s1)]pair
;;    [if (cCoIClosure u^) ([s1,u2]u2)]pair[if (cCoIClosure u^0) ([s1,u2]u2)])))

;; (pp "cCoIAvToAvc0CompRule")
...
;; For CoIAvcSatCoICl we need JKOne,JKTwo.

;; CoIAvcSatCoIClAuxJ
(set-goal "allnc d,e,i(Sd d -> Sd e -> Sdtwo i -> Sdtwo(JKOne(d+e+i*2)))")
...
(use "SdtwoInjElim"
     (pt "IntToSdtwo(JKOne(SdToInt s1+SdToInt s2+SdtwoToInt t*2))"))
(simp (pf "JKOne(SdToInt s1+SdToInt s2+SdtwoToInt t*2)=JKOne(d+e+i*2)"))
(use "SdtwoInjIntToSdtwo")
;; ?^27:abs(JKOne(d+e+i*2))<=2
(use "JProp")
...
;;  (IntToSdtwo
;;   (JKOne
;;    (SdToInt(cSdInjIntro s)+SdToInt(cSdInjIntro s0)+
;;     SdtwoToInt(cSdtwoInjIntro t)*2)))

(add-sound "CoIAvcSatCoIClAuxJ")

;; (pp (nf (theorem-name-to-formula "CoIAvcSatCoIClAuxJSound")))
...
;;   allnc t^(
;;    SdtwoMR i t^ -> 
;;    SdtwoMR(JKOne(d+e+i*2))
;;    (cSdtwoInjElim
;;     (IntToSdtwo
;;      (JKOne
;;       (SdToInt(cSdInjIntro s^)+SdToInt(cSdInjIntro s^0)+
;;        SdtwoToInt(cSdtwoInjIntro t^)*2)))))))

;; (pp "cCoIAvcSatCoIClAuxJ0CompRule")
...
;; CoIAvcSatCoIClAuxK
(set-goal "allnc d,e,i(Sd d -> Sd e -> Sdtwo i -> Sd(JKTwo(d+e+i*2)))")
(assume "d" "e" "i" "Sdd" "Sde" "Sdtwoi")
(assert "exl s1 SdInj d s1")
(use "SdInjIntro")
(use "Sdd")
...
(assume "ExHyp3")
(by-assume "ExHyp3" "t" "tProp")
(use "SdInjElim" (pt "IntToSd(JKTwo(SdToInt s1+SdToInt s2+SdtwoToInt t*2))"))
(simp (pf "JKTwo(SdToInt s1+SdToInt s2+SdtwoToInt t*2)=JKTwo(d+e+i*2)"))
(use "SdInjIntToSd")
;; ?^27:abs(JKTwo(d+e+i*2))<=1
(use "KProp")
;; ?^28:abs(d+e+i*2)<=6
...
;; (pp (nf (theorem-name-to-formula "CoIAvcSatCoIClAuxKSound")))
...
(pp "cCoIAvcSatCoIClAuxK0CompRule")
...
(use "ey1Prop")
(assume "CoIy1")
(intro 0 (pt "JKOne(d+e+i*2)"))
(intro 0 (pt "JKTwo(d+e+i*2)"))
(intro 0 (pt "x1"))
(intro 0 (pt "y1"))
(split)
...
(split)
(use "CoIy1")
;; ?^42:(1#4)*(x+y+i)===(1#2)*((1#4)*(x1+y1+JKOne(d+e+i*2))+JKTwo(d+e+i*2))
(simpreal "dx1Prop")
(simpreal "ey1Prop")
;; ?^44:(1#4)*((1#2)*(x1+d)+(1#2)*(y1+e)+i)===
;;      (1#2)*((1#4)*(x1+y1+JKOne(d+e+i*2))+JKTwo(d+e+i*2))
(use "RealEqSToEq")
(autoreal)
(cases (pt "x1"))
(assume "as" "M" "x1Def")
...
;;      (1#2)*((1#4)*(as n+bs n+JKOne(d+e+i*2))+JKTwo(d+e+i*2))
(simprat "RatTimesPlusDistr")
(simprat "RatTimesPlusDistr")
(simprat "RatTimesPlusDistr")
(simprat "RatTimesPlusDistr")
...
;;      (1#8)*as n+(1#8)*bs n+(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2)
(use "RatEqvTrans" (pt "(1#8)*as n+((d#8)+(1#8)*bs n+(e#8)+(i#4))"))
(use "Truth")
(use "RatEqvTrans" (pt "(1#8)*as n+((1#8)*bs n+(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2))"))
(use "RatPlusCompat")
(use "Truth")
...
(use "RatPlusCompat")
(use "Truth")
;; ?^79:(d#8)+((e#8)+(i#4))==(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)#2)
(simprat (pf "(i#4)==(i*2#8)"))
(simprat (pf "(JKTwo(d+e+i*2)#2)==(JKTwo(d+e+i*2)*4#8)"))
;; ?^82:(d#8)+((e#8)+(i*2#8))==(JKOne(d+e+i*2)#8)+(JKTwo(d+e+i*2)*4#8)
(simprat "<-" "RatEqvConstrPlus")
(simprat "<-" "RatEqvConstrPlus")
(simprat "<-" "RatEqvConstrPlus")
(simp (pf "JKOne(d+e+i*2)+JKTwo(d+e+i*2)*4=JKTwo(d+e+i*2)*4+JKOne(d+e+i*2)"))
(simp "KJProp")
(use "Truth")
(use "IntPlusComm")
;; ?^83:(JKTwo(d+e+i*2)#2)==(JKTwo(d+e+i*2)*4#8)
(ng #t)
(simp "<-" "IntTimesAssoc")
(use "Truth")
;; ?^81:(i#4)==(i*2#8)
...
;; [t,u,u0]
;;  cCoIAvcSatCoIClAuxJ[if (cCoIClosure u) ([s,u1]s)]
;;  [if (cCoIClosure u0) ([s,u1]s)]
;;  t pair 
;;  cCoIAvcSatCoIClAuxK[if (cCoIClosure u) ([s,u1]s)]
;;  [if (cCoIClosure u0) ([s,u1]s)]
;;  t pair
;;  [if (cCoIClosure u) ([s,u1]u1)]pair[if (cCoIClosure u0) ([s,u1]u1)]
...
;; JKOne(s1,s2,t) pair JKTwo(s1,s2,t) pair u1 pair u2.

(add-sound "CoIAvcSatCoICl")

;; (pp (nf (theorem-name-to-formula "CoIAvcSatCoIClSound")))
...
;;        tsuv^0))
;;      tsuv^))
;;    (cCoIAvcSatCoIClAuxJ[if (cCoIClosure u^) ([s1,u2]s1)]
;;     [if (cCoIClosure u^0) ([s1,u2]s1)]
;;     t^ pair 
;;     cCoIAvcSatCoIClAuxK[if (cCoIClosure u^) ([s1,u2]s1)]
;;     [if (cCoIClosure u^0) ([s1,u2]s1)]
;;     t^ pair
;;     [if (cCoIClosure u^) ([s1,u2]u2)]pair[if (cCoIClosure u^0) ([s1,u2]u2)]))))

;; (pp "cCoIAvcSatCoICl0CompRule")
...
(define CoIAvcToCoI-eterm (proof-to-extracted-term))
;; (add-var-name "tuv" (py "sdtwo yprod ai yprod ai"))
;; (add-var-name "tsuv" (py "sdtwo yprod sd yprod ai yprod ai"))
(define CoIAvcToCoI-neterm (rename-variables (nt CoIAvcToCoI-eterm)))
;; (pp CoIAvcToCoI-neterm)

;; [tuv]
;;  (CoRec sdtwo yprod ai yprod ai=>ai)tuv
;;  ([tuv0]
;;    [let tsuv
;;      (cCoIAvcSatCoICl[if tuv0 ([t,uv]t)][if tuv0 ([t,uv][if uv ([u,u0]u)])]
;;      [if tuv0 ([t,uv][if uv ([u,u0]u0)])])
;;      ([if tsuv ([t,suv][if suv ([s,uv]s)])]pair
;;      (InR (sdtwo yprod ai yprod ai) ai)
;;      ([if tsuv ([t,suv]t)]pair
;;       [if tsuv ([t,suv][if suv ([s,uv][if uv ([u,u0]u)])])]pair
;;       [if tsuv ([t,suv][if suv ([s,uv][if uv ([u,u0]u0)])])]))])

(animate "Id") ;needed for add-sound since let is present
(add-sound "CoIAvcToCoI")

(pp (nf (theorem-name-to-formula "CoIAvcToCoISound")))
...
;;  ((CoRec sdtwo yprod ai yprod ai=>ai)tuv^
;;   ([tuv0]
;;     [if (cCoIAvcSatCoICl[if tuv0 ([t1,uv2]t1)]
;;           [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u3)])]
;;           [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u4)])])
;;       ([t1,suv2][if suv2 ([s3,uv4]s3)])]pair
;;     (InR (sdtwo yprod ai yprod ai) ai)
;;     ([if (cCoIAvcSatCoICl[if tuv0 ([t1,uv2]t1)]
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u3)])]
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u4)])])
...
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u3)])]
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u4)])])
;;        ([t1,suv2][if suv2 ([s3,uv4][if uv4 ([u5,u6]u5)])])]pair
;;      [if (cCoIAvcSatCoICl[if tuv0 ([t1,uv2]t1)]
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u3)])]
;;            [if tuv0 ([t1,uv2][if uv2 ([u3,u4]u4)])])
;;        ([t1,suv2][if suv2 ([s3,uv4][if uv4 ([u5,u6]u6)])])]))))

;; (pp "cCoIAvcToCoI0CompRule")

;; cCoIAvcToCoI eqd
;; ([tuv]
;;   (CoRec sdtwo yprod ai yprod ai=>ai)tuv
;;   ([tuv0]
;;     [if (cCoIAvcSatCoICl[if tuv0 ([t,uv]t)][if tuv0 ([t,uv][if uv ([u,u0]u)])]
;;           [if tuv0 ([t,uv][if uv ([u,u0]u0)])])
;;       ([t,suv][if suv ([s,uv]s)])]pair
;;     (InR (sdtwo yprod ai yprod ai) ai)
;;     ([if (cCoIAvcSatCoICl[if tuv0 ([t,uv]t)]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u)])]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u0)])])
;;        ([t,suv]t)]pair
;;      [if (cCoIAvcSatCoICl[if tuv0 ([t,uv]t)]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u)])]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u0)])])
;;        ([t,suv][if suv ([s,uv][if uv ([u,u0]u)])])]pair
;;      [if (cCoIAvcSatCoICl[if tuv0 ([t,uv]t)]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u)])]
;;            [if tuv0 ([t,uv][if uv ([u,u0]u0)])])
;;        ([t,suv][if suv ([s,uv][if uv ([u,u0]u0)])])])))
...
;; Theorems used: (GfpCoIMR Id RealTimesReal RealRat RealPlusReal CoIToReal RealLeCompat RealEqSym RealAbsTimes RealEqRefl RealLeTrans RealLeMonTimesR RatLeToRealLe Truth RealLeAbsPlus RealLeMonPlusTwo SdtwoBoundReal CoIToBd RealEqCompat RealEqToReal0 CoIAvcSatCoIClSound)
...
;;    (cCoIAvcToCoI
;;    ([if tuv ([t,uv]t)]pair
;;     [if tuv ([t,uv][if uv ([u1,u2]u1)])]pair
;;     [if tuv ([t,uv][if uv ([u1,u2]u2)])]))]
...
(pp (nf (theorem-name-to-formula "CoIAverageSound")))
...
;;   CoIMR((1#2)*(x+y))
;;   (cCoIAvcToCoI
;;    ([if (cCoIAvToAvc u^ u^0) ([t1,uv2]t1)]pair
;;     [if (cCoIAvToAvc u^ u^0) ([t1,uv2][if uv2 ([u3,u4]u3)])]pair
;;     [if (cCoIAvToAvc u^ u^0) ([t1,uv2][if uv2 ([u3,u4]u4)])]))))

(pp "cCoIAverage0CompRule")
