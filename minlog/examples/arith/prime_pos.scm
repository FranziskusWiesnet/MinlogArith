;; 2026-01-27.  prime_pos.scm.  Written by Franziskus Wiesnet.
...
(add-computation-rules
 ...
 "PosLeastFactorAux(SZero p)"
 "NatToPos(NatLeast(PosSqrt(SZero p)+1)([n]1<n andb
  PosDiv (NatToPos n) (SZero p)))"
 "PosLeastFactorAux(SOne p)"
 "NatToPos(NatLeast(PosSqrt(SOne p)+1)([n]1<n andb
  PosDiv (NatToPos n) (SOne p)))")
...
;;PosGcdIdEqDExBProd
(set-goal "all p,q(PosGcd p q=p eqd ExBPos q([q1]q1*p=q))")
(strip)
(cases (pt "PosGcd p q=p"))
(assume 1)
(inst-with "PosGcdDiv1" (pt "q") (pt "p"))
(simphyp 2 1)
(inst-with "PosDivToProd" (pt "p") (pt "q") 3)
(by-assume 4 "p0" 5)
(assert "ExBPos q([q0]q0*p=q)")
(use "ExBPosIntro" (pt "p0"))
(simp "<-" 5)
(use "PosLeTrans" (pt "p0*1"))
(use "Truth")
(use "PosLeMonTimes")
(use "Truth")
(use "Truth")
...
(intro 0)
(assume 1)
(assert "ExBPos q([q0]q0*p=q)-> F")
(assume 2)
(use 1)
(use "ExBPosElim" (pt "q") (pt "[q0]q0*p=q"))
(use 2)
(assume "p0" 3 4)
(ng 4)
(simp "<-" 4)
...
(intro 0)
;;(cp)
(save "PosGcdIdEqDExBProd")

;;NatLeastCompat
(set-goal "all n,ws0,ws1(all n ws0 n=ws1 n -> NatLeast n ws0=NatLeast n ws1)")
(ind)
(search)
(assume "n" 1 "ws0" "ws1" 2)
(ng)
(simp 2)
(cases (pt "ws1 Zero"))
(search)
...
(assert "all p (NatLeast(PosToNat(PosSqrt p +1))([n]1<n andb NatToPos n|| p) =
         NatLeast(PosToNat(PosSqrt p +1)) ([n]1<n andb ExBPos p([q]q*n=PosToNat p)))")
(assume "p")
(use "NatLeastCompat")
(bpe-ng)
(assume "n")
(simp (pf "all p,q (PosDiv p q eqd NatDiv (PosToNat p) (PosToNat q))"))
(simp "NatDiv0CompRule")
(cases (pt "1<n"))
(assume 1)
(simp "AndConst0CompRule")
(simp "AndConst0CompRule")
(cases (pt "ExBPos p([q]q*n=PosToNat p)"))
(assume 2)
(assert "ExBNat(Succ(PosToNat p))([l]l*NatToPos n=PosToNat p)")
(use "ExBPosElim" (pt "p") (pt "[q]q*n=PosToNat p"))
(use 2)
(assume "q" 1 2)
(bpe-ng)
(use "ExBNatIntro" (pt "PosToNat q"))
(use "NatLeLtTrans" (pt "PosToNat p"))
(simp "PosToNatLe")
(use 3)
(use "Truth")
(bpe-ng)
...
(use "Truth")
(assume 2)
(assert "ExBNat(Succ(PosToNat p))([l]l*NatToPos n=PosToNat p) -> F")
(assume 3)
(use 2)
(use "ExBNatElim" (pt "[l]l*NatToPos n=PosToNat p") (pt "Succ(PosToNat p)"))
(use 3)
(assume "m" 4 5)
(bpe-ng)
(use "ExBPosIntro" (pt "NatToPos m"))
...
(cases (pt "q0||q1"))
(assume 1)
(assert "NatDiv (PosToNat q0) (PosToNat q1)")
(use "PosDivToNatDiv")
(use 1)
(assume 2)
(simp 2)
(intro 0)
(assume 1)
(assert "NatDiv (PosToNat q0) (PosToNat q1) -> F")
(assume 2)
(use 1)
(inst-with "NatDivToPosDiv" (pt "PosToNat q0") (pt "PosToNat q1"))
(simp "<-"  "NatToPosToNatId")
(simp "<-" (pf "NatToPos(PosToNat q1) = q1"))
(use 3)
(use "NatLtZeroPos")
(use "NatLtZeroPos")
(use 2)
...
(use "Truth")
(assume "p")
(simp "PosLeastFactorAux1CompRule")
(simp 1)
(use "Truth")
(assume "p")
(simp "PosLeastFactorAux2CompRule")
(simp 1)
(use "Truth")
;;(cp)
...
(add-computation-rules
 "PosLeastFactor 1"
 "1"
 "PosLeastFactor (SZero p)"
 "2"
 "PosLeastFactor (SOne p)"
 "[if (PosLeastFactorAux (SOne p)=PosSqrt (SOne p)+1) (SOne p) (PosLeastFactorAux (SOne p))]")
...
(use "TotalVar")
(assume "p")
(simp "PosLeastFactor2CompRule")
(use "TotalVar")
;; (cp)
(save-totality)

;; NatLeastFactorDouble
(set-goal "all n (Zero < n -> NatLeastFactor(NatDouble n)= 2)")
(cases)
(strip)
(use "Efq")
(use 1)
...
(split)
(use "Truth")
(use "ExBNatIntro" (pt "Succ n"))
(ng)
(use "NatLtLeTrans" (pt "Succ(Succ n)"))
...
(use "Truth")
(assume "p")
(simp "PosLeastFactor1CompRule")
(simp "PosToNat1CompRule")
(simp "PosToNat1CompRule")
(simp "NatLeastFactorDouble")
(use "Truth")
(use "NatLtZeroPos")
(assume "p")
(simp "PosLeastFactor2CompRule")
(simp "NatLeastFactor0CompRule")
(cases (pt "PosLeastFactorAux (SOne p)=PosSqrt (SOne p)+1"))
;; if (SOne p) is prime
(assume 1)
(simp "IfTrue")
(inst-with "PosLeastFactorAuxToPosPrime" (pt "SOne p") "Truth" 1)
(use "NatLeAntiSym")
(use "NatNotLtToLe")
(assume 3)
(inst-with
 "NatLeastLtElim"
 (pt "PosToNat (SOne p)")
 (pt "[m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p))")
 3)
(bpe-ng)
(use "ExBNatElim"
     (pt "[l]
      l*
      NatLeast(PosToNat (SOne p))
      ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l0]l0*m=PosToNat (SOne p)))=
      PosToNat (SOne p)")
     (pt "Succ(PosToNat (SOne p))"))
(use 4)
(assume "m" 5 6)
(bpe-ng)
(inst-with
 ...
 (pt "PosToNat (SOne p)")
 (pt "m")
 (pt "NatLeast(PosToNat (SOne p))
      ([m0]Succ Zero<m0 andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m0=PosToNat (SOne p)))")
 "?"
 6)
(cases (pt "m=Succ Zero"))
(assume 8)
(simphyp 6 8)
(assert "PosToNat (SOne p) <PosToNat (SOne p)")
(use "NatLeLtTrans" (pt "NatLeast(PosToNat (SOne p))
     ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p)))"))
(simp (pf "NatLeast(PosToNat (SOne p))
     ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p)))=
     Succ Zero*
     NatLeast(PosToNat (SOne p))
     ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p)))"))
...
;; Assertion 1
(assert "Succ Zero<
    NatLeast(PosToNat(PosSqrt (SOne p)+1))
     ([n]1<n andb ExBPos (SOne p)([q]q*n=PosToNat (SOne p)))")
(use "NatNotLeToLt")
(assume 4)
(inst-with
 "NatLeastLtElim"
 (pt "PosToNat(PosSqrt (SOne p)+1)")
 (pt "[n]1<n andb ExBPos (SOne p)([q]q*n=PosToNat (SOne p))")
 "?")
 (bpe-ng)
 (assert "Succ Zero<Succ Zero")
(use "NatLtLeTrans"
     (pt "NatLeast(PosToNat(PosSqrt (SOne p)+1))([n]1<n andb
          ExBPos (SOne p)([q]q*n=PosToNat (SOne p)))"))
 (use 3)
 (use 2)
 (search)
...
(inst-with
 ...
 (bpe-ng)
 (assert "Succ Zero<Succ Zero")
 (use "NatLtLeTrans" (pt "NatLeast(PosToNat (SOne p))
    ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p)))"))
 (use 4)
 (use 3)
 (search)
...
(assert "NatToPos
    (NatLeast(PosToNat(PosSqrt (SOne p)+1))
     ([n]1<n andb ExBPos (SOne p)([q]q*n=PosToNat (SOne p))))< PosSqrt (SOne p)+1")
 (use "PosLeNotEqToLt")
 (use "PosLeTrans" (pt "NatToPos(PosToNat(PosSqrt (SOne p)+1))"))
 (simp "NatToPosLe")
 (use "NatLeastBound")
 (use "NatLtZeroPos")
 (use "NatLtTrans" (pt "Succ Zero"))
 ...
 (assume 4)
;; Assertion 4
(assert "ExBPos (SOne p)
    ([q]
      q*
      NatLeast(PosToNat(PosSqrt (SOne p)+1))
      ([n]1<n andb ExBPos (SOne p)([q0]q0*n=PosToNat (SOne p)))=
      PosToNat (SOne p))")
...
(bpe-ng)
;; Assertion 5
(assert " ExBNat(Succ(PosToNat (SOne p)))
     ([l]
       l*
       NatLeast(PosToNat (SOne p))
       ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l0]l0*m=PosToNat (SOne p)))=
...
(bpe-ng)
;; Assertion 6
(assert "NatLeast(PosToNat (SOne p))
    ([m]Succ Zero<m andb ExBNat(Succ(PosToNat (SOne p)))([l]l*m=PosToNat (SOne p)))<
    PosToNat (SOne p)")
 (use "NatLeLtTrans" (pt "NatLeast(PosToNat(PosSqrt (SOne p)+1))
    ([n]1<n andb ExBPos (SOne p)([q0]q0*n=PosToNat (SOne p)))"))
 (use "NatLeastLeIntro")
 (bpe-ng)
 (split)
 ...
 (use 6)
 (use 7)
 (use "NatLtLeTrans" (pt "PosToNat(PosSqrt (SOne p)+1)"))
 (simp "<-" "NatToPosLt")
 (simp "NatToPosToNatId")
 (use 4)
 (use "NatLtZeroPos")
 ...
 (use "Truth")
 (use 2)
 (simp (pf "PosToNat(PosSqrt (SOne p)+1)=Succ(PosToNat(PosSqrt (SOne p)))"))
 (use "NatLtToSuccLe")
 (simp "PosToNatLt")
 (use "PosLtSqrt")
 (use "Truth")
...
(use 14)
(use 11)
(use "NatLeTrans" (pt "Succ Zero *(SOne p)"))
(use "NatLeMonTimes")
(use 12)
(use "Truth")
(use "Truth")
...
;;PosListToNewPrime
(set-goal "all m,ps(exl p(PosPrime p andnc PosNewNumber p ps m))")
(assume "m" "ps")
