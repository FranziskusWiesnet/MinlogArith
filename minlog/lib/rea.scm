...
;; all as^,as^0(
;;  all n^,n^0(EqPNat n^ n^0 -> EqPRat(as^ n^)(as^0 n^0)) -> 
;;  all M^,M^0(
;;   all p^,p^0(EqPPos p^ p^0 -> EqPNat(M^ p^)(M^0 p^0)) -> 
...
(add-ids
 ...
 '("all as^(all n^(TotalNat n^ -> TotalRat(as^ n^)) ->
    all M^(all p^(TotalPos p^ -> TotalNat(M^ p^)) ->
    TotalRea(RealConstr as^ M^)))"
   "TotalReaRealConstr"))

(add-ids
 (list (list "TotalReaNc" (make-arity (py "rea"))))
 '("all as^(all n^(TotalNatNc n^ -> TotalRatNc(as^ n^)) ->
    all M^(all p^(TotalPosNc p^ -> TotalNatNc(M^ p^)) ->
    TotalReaNc(RealConstr as^ M^)))"
   "TotalReaNcRealConstr"))

;; EqPTotalNatToRatLeft
(set-goal "all as^1,as^2(
     all n^1,n^2(EqPNat n^1 n^2 -> EqPRat(as^1 n^1)(as^2 n^2)) -> 
     all n^(TotalNat n^ -> TotalRat(as^1 n^)))")
...
(set-goal "all as^1,as^2(
     all n^1,n^2(EqPNat n^1 n^2 -> EqPRat(as^1 n^1)(as^2 n^2)) -> 
     all n^(TotalNat n^ -> TotalRat(as^2 n^)))")
(assume "as^1" "as^2" "EqPas1as2" "n^" "Tn")
...
(set-goal "all M^1,M^2(
     all p^,p^0(EqPPos p^ p^0 -> EqPNat(M^1 p^)(M^2 p^0)) -> 
     all p^(TotalPos p^ -> TotalNat(M^1 p^)))")
...
(set-goal "all M^1,M^2(
     all p^,p^0(EqPPos p^ p^0 -> EqPNat(M^1 p^)(M^2 p^0)) -> 
     all p^(TotalPos p^ -> TotalNat(M^2 p^)))")
(assume "M^1" "M^2" "EqPM1M2" "p^" "Tp")
...
;; EqPReaToTotalLeft
(set-goal "all x^,y^(EqPRea x^ y^ -> TotalRea x^)")
(assume "x^" "y^" "EqPxy")
(elim "EqPxy")
...
;; EqPReaToTotalRight
(set-goal "all x^,y^(EqPRea x^ y^ -> TotalRea y^)")
(assume "x^" "y^" "EqPxy")
(elim "EqPxy")
(assume "as^1" "as^2" "EqPas1as2" "M^1" "M^2" "EqPM1M2")
(use "TotalReaRealConstr")
...
;; EqPReaRefl
(set-goal "all x^(TotalRea x^ -> EqPRea x^ x^)")
(assume "x^" "Tx")
(elim "Tx")
(assume "as^" "Tas" "M^" "TM")
...
;; EqPReaAux reduces a goal all x^,y^(EqPRea x^ y^ -> (Pvar rea rea)x^ y^)
...
(set-goal "all as^,M^,bs^,N^(
 ...
 all x^,y^(EqPRea x^ y^ -> (Pvar rea rea)x^ y^)")
...
;; EqPReaElimLeft
(set-goal "all x^,y^(EqP x^ y^ -> EqP(x^ seq)(y^ seq))")
(assume "x^" "y^" "EqPxy")
(elim "EqPxy")
(assume "as^1" "as^2" "EqPas1as2" "M^1" "M^2" "EqPM1M2")
(ng #t)
...
;; 2020-07-13.  Changed because of pure unfolding of the premise.
;; EqPReaElimRight
(set-goal "all x^,y^(EqP x^ y^ -> EqP(x^ mod)(y^ mod))")
(assume "x^" "y^" "EqPxy")
(elim "EqPxy")
(assume "as^1" "as^2" "EqPas1as2" "M^1" "M^2" "EqPM1M2")
(ng #t)
...
(set-goal "all as^,M^,bs^,N^(
 EqPRea(RealConstr as^ M^)(RealConstr bs^ N^) ->
 all n^,m^(EqPNat n^ m^ -> EqPRat(as^ n^)(bs^ m^)))")
...
(set-goal "all as^,M^,bs^,N^(
 EqPRea(RealConstr as^ M^)(RealConstr bs^ N^) ->
 all p^,q^(EqPPos p^ q^ -> EqPNat(M^ p^)(N^ q^)))")
(assume "as^" "M^" "bs^" "N^" "EqPxy")
;; (use "AllEqPPosElim")
...
;; ListNatMaxEqP
(set-goal "all nl^1,nl^2(EqP nl^1 nl^2 ->
...
(add-computation-rules
 ...
 "RealConstr([n]as n*bs n)
            ([p]M(PosS(p+NatToPos(cRBd bs N)))max
                N(PosS(p+NatToPos(cRBd as M))))")
...
;; TotalReaNcToEqD
(set-goal "all x^(TotalReaNc x^ -> x^ eqd RealConstr x^ seq x^ mod)")
(assume "x^")
(elim)
(ng)
(strip)
...
(set-goal "all as,M,bs,N(RealConstr as M<<=RealConstr bs N -> 
      all p exnc n1 all n(n1<=n -> as n<=bs n+(1#2**p)))")
...
;; RealCompat
(set-goal "all x,y(x===y -> Real x -> Real y)")
(assume "x" "y" "EqH" "Rx")
...
(use-with "CauchyElim"
	  (pt "[n]as n*bs n")
	  (pt "[p]M(PosS(p+NatToPos(cRBd bs N)))max
                  N(PosS(p+NatToPos(cRBd as M)))")
	  "?" (pt "p") (pt "n") (pt "m") "?" "?")
(use "CauchyTimes")
(use "CasM")
...
(use "MonM")
(use "MonN")
;; ?^23:all n abs(as n)<=2**NatToPos(cRBd as M)
...
(assert "PosToNat(NatToPos(cRBd as M))=cRBd as M")
(use "PosToNatToPosId")
(use "xBdPos")
;; Assertion proved.
(assume "EqHyp")
...
(use "RealBdProp")
(use "CasM")
;; ?^24:all n abs(bs n)<=2**NatToPos(cRBd bs N)
...
(assert "PosToNat(NatToPos(cRBd bs N))=cRBd bs N")
(use "PosToNatToPosId")
(use "yBdPos")
;; Assertion proved.
(assume "EqHyp")
...
(use "NatMaxLUB")
;; 51,52
(use "NatLeTrans" (pt "M(PosS(q+NatToPos(cRBd bs N)))"))
(use "MonElim")
(use "MonM")
(ng #t)
(use "p<=q")
(use "NatMaxUB1")
;; 52
(use "NatLeTrans" (pt "N(PosS(q+NatToPos(cRBd as M)))"))
(use "MonElim")
(use "MonN")
(ng #t)
(use "p<=q")
...
(assert "all x,y(Real x -> Real y -> all n (x*y)seq n==x seq n*y seq n)")
(cases)
(assume "as" "M")
(cases)
...
(assert "exnc n3 all n(n3<=n -> ([n](0#1))n<=as n+(1#2**p))")
(use "RealLeChar1" (pt "[p]Zero") (pt "M"))
(simp "<-" "xDef")
(use "0<=x")
;; Assertion proved.
(ng #t)
;; ?^66:exnc n all n0(n<=n0 -> 0<=as n0+(1#2**p)) -> 
;;      exnc n all n0(n<=n0 -> 0<=as n0*bs n0+(1#2**p))
(assume "n3Ex")
(by-assume "n3Ex" "n3" "n3Prop")

(assert "exnc n5 all n0(n5<=n0 -> ~(1#2**p)<=as n0)")
(intro 0 (pt "n3"))
(assume "m")
...
(assert "exnc n4 all n(n4<=n -> ([n](0#1))n<=bs n+(1#2**p))")
(use "RealLeChar1" (pt "[p]Zero") (pt "N"))
(simp "<-" "yDef")
(use "0<=y")
;; Assertion proved.
(ng #t)
;; ?^82:exnc n all n0(n<=n0 -> 0<=bs n0+(1#2**p)) -> 
;;      exnc n all n0(n<=n0 -> 0<=as n0*bs n0+(1#2**p))
(assume "n4Ex")
(by-assume "n4Ex" "n4" "n4Prop")

(assert "exnc n6 all n0(n6<=n0 -> ~(1#2**p)<=bs n0)")
(intro 0 (pt "n4"))
(assume "m")
(simp "<-" "RatZeroLePlusEqUMinusLe")
(use "n4Prop")
...
;;   {n3}  aProp:exnc n all n0(n<=n0 -> ~(1#2**p)<=as n0)
;;   {n4}  bProp:exnc n all n0(n<=n0 -> ~(1#2**p)<=bs n0)
;; -----------------------------------------------------------------------------
...
(inst-with-to "RealEqChar1"
              (pt "x") (pt "y") "x=y" (pt "p+NatToPos(cRBd cs K)") "nEx")
(by-assume "nEx" "n" "nProp")
(intro 0 (pt "n"))
(assume "m" "n<=m")
...
(simp "RatAbsTimes")
(use "RatLeTrans"
     (pt "(1#2**(p+NatToPos(cRBd cs K)))*(2**cRBd cs K)"))
(use "RatLeMonTimesTwo")
(use "Truth")
(use "Truth")
;; ?^37:abs(as m+ ~(bs m))<=(1#2**(p+NatToPos(cRBd cs K)))
(use "RatLeTrans" (pt "abs(x seq m+ ~(y seq m))"))
;; 39,40
(simp "xDef")
(simp "yDef")
...
(ng #t)
(simp "<-" "PosExpTwoPosPlus")
;; ?^56:2**l*2**p<=2**p*2**NatToPos l
(assert "PosToNat(NatToPos(cRBd cs K))=cRBd cs K")
(use "PosToNatToPosId")
(use "RealBdPos")
(assume "EqHyp")
(simp "lDef")
...
;; MonStab
(set-goal "all M(((Mon M -> F) -> F) -> Mon M)")
(assume "M" "u")
(use "MonIntro")
(assume "p" "q" "u0")
(use "StabAtom")
...
(assume "u2")
(use "u1")
(use-with "MonElim" (pt "M") "u2" (pt "p") (pt "q") "u0")
;; Proof finished.
;; (cp)
(save "MonStab")

;; CauchyStab
(set-goal "all as,M(((Cauchy as M -> F) -> F) -> Cauchy as M)")
(assume "as" "M" "u")
(use "CauchyIntro")
(assume "p" "n" "m" "u0" "u1")
(use "StabAtom")
...
;; RealStab
(set-goal "all x(((Real x -> F) -> F) -> Real x)")
(assume "x" "u")
(use "RealIntro")
;; 3,4
(use "CauchyStab")
...
;; RealAbsLe
(set-goal "all x,y(Real x -> RealAbs x<<=y -> x<<=y)")
(assume "x" "y" "Rx" "yBd")
...
;; all x,p(
;;  Real x -> (ExLTMR (cterm (a) abs(a+ ~x)<<=(1#2**p)))(x seq(x mod p)))
...
;; (pp (nf (theorem-name-to-formula "ApproxSplitPosSound")))
...
;;        [if z
;;          ([as4,M5]
;;           as4(M3(PosS(PosS p))max M1(PosS(PosS p))max M5(PosS(PosS p)))<=
;;           (1#2)*
;;           (as0(M3(PosS(PosS p))max M1(PosS(PosS p)))+
;;            as2(M3(PosS(PosS p))max M1(PosS(PosS p)))))])])])
...
;; (pp (nf (theorem-name-to-formula "ApproxSplitSound")))
...
;; RealAllTotalIntro
(set-goal "all as,M (Pvar rea)(RealConstr as M) -> all x (Pvar rea)x")
(assume "asMHyp")
(use "AllTotalIntro")
(assume "x^" "Tx")
(simp "TotalReaToEqD")

(assert "all as^(Total as^ -> all M (Pvar rea)(RealConstr as^ M))")
(use-with "AllTotalElim" (py "nat=>rat")
 (make-cterm (pv "as^") (pf "all M (Pvar rea)(RealConstr as^ M)")) "?")
(use "asMHyp")
;; Assertion proved.
(assume "Assertion")
(inst-with-to "Assertion" (pt "x^ seq") "AssInst")
(drop "Assertion")
(assert "all M^(Total M^ -> (Pvar rea)(RealConstr x^ seq M^))")
(use-with "AllTotalElim" (py "pos=>nat")
...
;; Proof finished.
;; (cp)
(save "RealAllTotalIntro")

(define eterm (proof-to-extracted-term))
(define neterm (rename-variables (nt eterm)))
;; (pp neterm)
;; [((nat=>rat)=>(pos=>nat)=>alpha3638),x]
;;  ((nat=>rat)=>(pos=>nat)=>alpha3638)[if x ([as,M]as)][if x ([as,M]M)]

;; RealAllTotalElim
(set-goal "all x (Pvar rea)x -> all as,M (Pvar rea)(RealConstr as M)")
(assume "xHyp")
(assert "all x^(TotalRea x^ -> (Pvar rea)x^)")
 (use-with "AllTotalElim" (py "rea")
...
;; Proof finished.
;; (cp)
(save "RealAllTotalElim")

(define eterm (proof-to-extracted-term))
(define neterm (rename-variables (nt eterm)))
;; (pp neterm)
;; [(rea=>alpha3638),as,M](rea=>alpha3638)(RealConstr as M)
...
(assert
 "all p exnc n all n0(n<=n0 -> ([n]as n+bs n)n0<=([n]as n+cs n)n0+(1#2**p))")
(use "RealLeChar1" (pt "([p]M(PosS p)max N(PosS p))")
     (pt "([p]M(PosS p)max K(PosS p))"))
(use "x+y<<=x+z")
(ng #t)
;; ?^16:all p exnc n all n0(n<=n0 -> as n0+bs n0<=as n0+cs n0+(1#2**p)) -> 
...
(autoreal)
(use "RealLeCompat"
 (pt "RealConstr([n]1#2**(PosS p))([p] Zero)") (pt "y + ~x"))
;; 7-9
(simpreal "RealPlusAssoc")
(simpreal (pf "~x+x===x+ ~x"))
(simpreal "RealPlusMinusZero")
...
;; RealEqTimesCancelL
(set-goal "all x,y,z,p(
 Real x -> Real y -> Real z -> RealPos x p -> x*y===x*z -> y===z)")
(assume "x" "y" "z" "p" "Rx" "Ry" "Rz" "PosHyp" "x*y===x*z")
(use "RealLeAntiSym")
...
;; RealEqTimesCancelR
(set-goal "all x,y,z,p(
 Real x -> Real y -> Real z -> RealPos z p -> x*z===y*z -> x===y)")
...
(set-goal
 "all x,p,n(Real x -> x mod p<=n -> abs(x seq n+ ~x)<<=(1#2**p))")
(cases)
(assume "as" "M" "p" "n" "Rx" "nBd")
...
;; RealCauchyProp
(set-goal "all as,M(Cauchy as M ->
 all p,n((1#2**p)<=as(M(PosS p)) -> M(PosS p)<=n -> (1#2**PosS p)<=as n))")
(assume "as" "M" "CasM" "p" "n" "PosHyp" "nBd")
(use "RatLePlusCancelR" (pt "(1#2**PosS p)"))
(simprat "RatPlusHalfExpPosS")
(simp "RatPlusComm")
(simprat "<-" (pf "(1#2**p)+ ~(as n)+as n==(1#2**p)"))
(use "RatLeMonPlus")
(use "RatLeTrans" (pt "as(M(PosS p))+ ~(as n)"))
(use "RatLeMonPlus")
(use "PosHyp")
(use "Truth")
(use "RatLeTrans" (pt "abs(as(M(PosS p))+ ~(as n))"))
...
;; RealLeZeroToNotPos
(set-goal "all x,p(x<<=0 -> RealPos x p -> F)")
(cases)
(assume "as" "M" "p" "x<=0" "PosHyp")
(inst-with-to "RealLeChar1RealConstrFree"
	      (pt "RealConstr as M")
	      (pt "RealConstr([n](0#1))([p]Zero)")
	      "x<=0" (pt "PosS(PosS p)") "ExHyp")
(ng)
(by-assume "ExHyp" "n1" "n1Prop")
(assert "all n(n1<=n -> M(PosS p)<=n -> as n<as n)")
(assume "n" "n1<=n" "nBd")
(use "RatLeLtTrans" (pt "(1#2**PosS(PosS p))"))
;; 13,14
(use "n1Prop")
(use "n1<=n")
;; 14
...
;; Assertion proved.
(assume "Assertion")
(use-with "Assertion" (pt "n1 max M(PosS p)") "?" "?")
(use "NatMaxUB1")
(use "NatMaxUB2")
;; Proof finished.
;; (cp)
(save "RealLeZeroToNotPos")

;; RealHalfPlus
(set-goal "all x(Real x -> x===(1#2)*(x+x))")
(assume "x" "Rx")
(simpreal (pf "x+x===2*x"))
(simpreal "RealTimesAssoc")
(ng #t)
...
;; RealAbsAvBd
(set-goal "all x,y,z(Real x -> Real y -> abs x<<=z -> abs y<<=z ->
 abs((1#2)*(x+y))<<=z)")
...
;; Proof finished.
;; (cp)
(save "RealLeZeroTimes")

;; RealLeTimesZero
(set-goal "all x,y(x<<=0 -> 0<<=y -> x*y<<=0)")
(assume "x" "y" "x<=0" "0<=y")
(use "RealLeUMinusInv")
(ng #t)
;; ?^4:0<<= ~(x*y)
...
;; Proof finished.
;; (cp)
(save "RealLeTimesZero")

;; RealZeroNotPos
(set-goal "all p RealPos(RealConstr([n](0#1))([p]Zero))p=False")
(assume "p")
(use "Truth")
;; Proof finished.
;; (cp)
(add-rewrite-rule "RealPos(RealConstr([n](0#1))([p]Zero))p" "False")
(save "RealZeroNotPos")

;; RealNotLeToExca
(set-goal "all x,y(
 Real x -> Real y -> (x<<=y -> F) -> exca p RealPos(x+ ~y)p)")
(assume "x" "y" "Rx" "Ry" "xyHyp")
(assume "AllHyp")
(use "xyHyp")
(use "RealNotLtToLe")
(use "Rx")
...
;; Proof finished.
;; (cp)
(save "RealNotLeToExca")

;; RealLeCasesStab
(set-goal "(((Pvar^ -> F) -> F) -> Pvar^) -> all x,y(Real x -> Real y ->
 (x<<=y -> Pvar^) -> (y<<=x -> Pvar^) -> Pvar^)")
(assume "StabPvar" "x" "y" "Rx" "Ry" "xyHyp" "yxHyp")
(use "StabPvar")
(assume "NotPvar")
...
(inst-with-to "RealNotLeToExca" (pt "x") (pt "y") "Rx" "Ry" "x<|=y" "xyProp")
(inst-with-to "RealNotLeToExca" (pt "y") (pt "x") "Ry" "Rx" "y<|=x" "yxProp")
(by-assume "xyProp" "p" "pProp")
(by-assume "yxProp" "q" "qProp")
(assert "RealPos((x+ ~y)+(y+ ~x))(PosS(PosS(p min q)))")
(use "RealPosMonPlus")
;; 29-32
(autoreal)
;; 31
...
(use "qProp")
(assume "PosHyp")
(assert "RealPos 0(PosS(PosS(PosS(PosS(p min q)))))")
(use "RealPosSemiCompat" (pt "(x+ ~y+(y+ ~x))"))
(simpreal "RealPlusAssoc")
...
;; Proof finished.
;; (cp)
(save "RealLeCasesStab")

;; As a special case we obtain

;; RealLeCases
(set-goal "all x,y(Real x -> Real y -> (x<<=y -> F) -> (y<<=x -> F) -> F)")
(use "RealLeCasesStab")
(assume "u")
(use "u")
(assume "v")
...
;; RealBound : rea=>nat instead of RealBd : (nat=>rat)=>(pos=>nat)=>nat

(add-program-constant "RealBound" (py "rea=>nat"))

(add-computation-rules
 "RealBound x"
 "Succ(ListNatMax(cRatLeAbsBound map x seq fbar Succ(x mod 1)))")
...
;; RBound
(set-goal "all x(Real x -> exl n n=RealBound x)")
(assume "x" "Rx")
(intro 0 (pt "RealBound x"))
(use "Truth")
;; Proof finished.
;; (cp)
(save "RBound")
...
;; BoundExFree
(set-goal "all x(Real x -> cRBound x=RealBound x)")
(assume "x" "Rx")
(use "Truth")
;; Proof finished.
;; (cp)
...
;; Now we can use the deanimated cRBound instead of RealBound.

(animate "RBound")
(animate "RBd")

;; EqRBdRBound
(set-goal "all x cRBd x seq x mod=cRBound x")
(cases)
(assume "as" "M")
(use "Truth")
;; Proof finished.
...
;; RealBoundProp
(set-goal "all x(Real x -> all n abs(x seq n)<=2**cRBound x)")
(assume "x" "Rx" "n")
(use "RatLeTrans" (pt "(2**cRBd x seq x mod#1)"))
;; 3,4
(use "RealBdProp")
(use "RealToCauchy")
(use "Rx")
...
;; RealBoundProp1
(set-goal "all x(Real x -> abs x<<=2**cRBound x)")
(cases)
(assume "as" "M" "Rx")
(inst-with-to "RealBoundProp" (pt "RealConstr as M") "Rx" "Inst")
(ng "Inst")
(use "RealLeChar2RealConstrFree")
(get 9)
(ng #t)
...
(intro 0 (pt "Zero"))
(assume "n" "Useless")
(use "RatLeTrans" (pt "(2**cRBound(RealConstr as M)#1)"))
(use "Inst")
(ng #t)
(use "Truth")
;; 7
...
;; RealBoundProp2
(set-goal "all x(Real x -> abs x<<=RealTimes(1#2)(2**Succ(cRBound x)))")
(assume "x" "Rx")
(use "RealLeTrans" (pt "RealConstr([n]2**cRBound x)([p]Zero)"))
(use "RealBoundProp1")
(use "Rx")
(ng #t)
(use "RatLeToRealLe")
...
;; RealTimesExp
(set-goal "all x,y(Real x -> Real y -> all n x**n*y**n===(x*y)**n)")
(assume "x" "y" "Rx" "Ry")
(ind)
;; 3,4
(ng #t)
...
;; RealLeToPosZero
(set-goal "all x(1<<=x -> RealPos x 1)")
(cases)
(assume "as" "M" "LeHyp")
(ng #t)
;; (elim "LeHyp")
(inst-with-to
 "RealLeElim2"
 (pt "RealConstr([n]1)([p]Zero)")
 (pt "RealConstr as M")
 "LeHyp"
 (pt "1")
 "Inst")
(ng "Inst")
(use "RatLePlusCancelR" (pt "1#2"))
(ng #t)
(use "Inst")
;; Proof finished.
;; (cp)
(save "RealLeToPosZero")
