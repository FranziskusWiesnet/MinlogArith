...
;; IntEqToEqD
(set-goal "all k,k0(k=k0 -> k eqd k0)")
(cases) 
;; 2-4
(assume "p")
(cases)
...
(set-goal "all k^(TotalInt k^ ->
 ...
 Total alpha^ ->
 all p^(TotalPos p^ -> Total((pos=>alpha)^1 p^)) ->
 all p^(TotalPos p^ -> Total((pos=>alpha)^2 p^)) ->
...
;; EqPIntToTotalLeft
(set-goal "all k^,k^0(EqPInt k^ k^0 -> TotalInt k^)")
(assume "k^" "k^0" "EqPkj")
(elim "EqPkj")
;; 3-5
...
;; EqPIntToTotalRight
(set-goal "all k^,k^0(EqPInt k^ k^0 -> TotalInt k^0)")
(assume "k^" "k^0" "EqPkj")
(elim "EqPkj")
;; 3-5
(assume "p^1" "p^2" "EqPp1p2")
...
;; EqPIntToEqD
(set-goal "all k^,k^0(EqPInt k^ k^0 -> k^ eqd k^0)")
(assume "k^" "k^0" "EqPkj")
(elim "EqPkj")
;; 3-5
(assume "p^1" "p^2" "EqPp1p2")
...
;; EqPIntRefl
(set-goal "all k^(TotalInt k^ -> EqPInt k^ k^)")
(assume "k^" "Tk")
(elim "Tk")
;; 3-5
(assume "p^" "Tp")
...
;; EqPIntSym
(set-goal "all k^,k^0(EqPInt k^ k^0 -> EqPInt k^0 k^)")
(assume "k^" "k^0" "EqPkj")
(elim "EqPkj")
(assume "p^" "q^" "EqPpq")
(use "EqPIntIntPos")
...
;; IntEqTotal
(set-goal "all k^(
 TotalInt k^ -> all k^0(TotalInt k^0 -> TotalBoole(k^ =k^0)))")
(assume "k^" "Tk")
(elim "Tk")
;; 3-5
(assume "p^" "Tp" "k^0" "Tk0")
...
;; ?_3:TotalInt Zero
(use "TotalIntIntZero")
;; ?_4:all nat^(TotalNat nat^ -> TotalInt nat^ -> TotalInt(Succ nat^))
(assume "m^" "Tm" "IH")
(ng #t)
;; ?_6:TotalInt(IntS m^)
(use "IntSTotal")
...
;; IntSInj
(set-goal "all k,k0(IntS k=IntS k0 -> k=k0)")
(assume "k" "k0" "Sk=Sk0")
(assert "IntPred(IntS k)=IntPred(IntS k0)")
 (simp "Sk=Sk0")
 (use "Truth")
(ng)
(assume "k=k0")
...
;; IntPredInj
(set-goal "all k,k0(IntPred k=IntPred k0 -> k=k0)")
(assume "k" "k0" "Pk=Pk0")
(assert "IntS(IntPred k)=IntS(IntPred k0)")
 (simp "Pk=Pk0")
 (use "Truth")
(ng)
(assume "k=k0")
...
;; IntPlusCompat
(set-goal "all k,k0,k1,k2(k=k0 -> k1=k2 -> k+k1=k0+k2)")
(assume "k" "k0" "k1" "k2" "k=k0" "k1=k2")
(simp "k=k0")
(simp "k1=k2")
(use "Truth")
;; Proof finished.
...
;; IntPlusAssoc
(set-goal "all k,k0,k1 k+(k0+k1)=k+k0+k1")
;; IntPlusAssocPPN
(assert "all p,q,r
...
(add-rewrite-rule "k+(k0+k1)" "k+k0+k1")
...
;; Proof finished.
;; (cp)
;; (add-rewrite-rule "IntN(SOne p)+k" "IntPred(k+IntN(SZero p))")

;; IntPlusIntSId
(set-goal "all k,k0 IntS k+k0=IntS(k+k0)")
(assume "k" "k0")
(simp "<-" "IntPlusOneId")
(simp "<-" "IntPlusOneId")
(simp "IntPlusAssoc")
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "IntS k+k0" "IntS(k+k0)")

;; IntPlusIdIntS
(set-goal "all k,k0 k+IntS k0=IntS(k+k0)")
(assume "k" "k0")
(simp "<-" "IntPlusIdOne")
(simp "<-" "IntPlusIdOne")
(simp "IntPlusAssoc")
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "k+IntS k0" "IntS(k+k0)")
...
;; IntTimesPlusDistr
(set-goal "all k,k0,k1 k*(k0+k1)=k*k0+k*k1")
...
(add-rewrite-rule "k*(k0+k1)" "k*k0+k*k1")

;; IntTimesPlusDistrLeft
(set-goal "all k,k0,k1 (k+k0)*k1=(k*k1)+(k0*k1)")
(assume "k" "k0" "k1")
(simp "IntTimesComm")
...
(add-rewrite-rule "(k+k0)*k1" "(k*k1)+(k0*k1)")
...
(set-goal "all k,k0 abs(k*k0)=abs k*abs k0")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "abs(k*k0)" "abs k*abs k0")
...
;; IntTimesCancelL
(set-goal "all k,k0,k1(0<abs k -> k*k0=k*k1 -> k0=k1)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntTimesCancelR
(set-goal "all k,k0,k1(0<abs k1 -> k*k1=k0*k1 -> k=k0)")
(assume "k" "k0" "k1" "PosHyp" "kk1=k0k1")
(use "IntTimesCancelL" (pt "k1"))
(use "PosHyp")
(simp "IntTimesComm")
(simp "kk1=k0k1")
(use "IntTimesComm")
...
;; IntLtTrans
(set-goal "all k,k0,k1(k<k0 -> k0<k1 -> k<k1)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLeAntiSym
(set-goal "all k,k0(k<=k0 -> k0<=k -> k=k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLtMonIntS
(set-goal "all k,k0(k<k0 -> IntS k<IntS k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLtMonIntPred
(set-goal "all k,k0(k<k0 -> IntPred k<IntPred k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
(use "BooleAeqToEq")
;; 3,4
(assume "IntS k<IntS k0")
(simp "<-" (pf "IntPred(IntS k)=k"))
(simp "<-" (pf "IntPred(IntS k0)=k0"))
(use "IntLtMonIntPred")
(use "IntS k<IntS k0")
(use "Truth")
(use "Truth")
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "IntS k<IntS k0" "k<k0")

(set-goal "all k,k0 (IntPred k<IntPred k0)=(k<k0)")
(assume "k" "k0")
(use "BooleAeqToEq")
;; 3,4
(assume "IntPred k<IntPred k0")
(simp "<-" (pf "IntS(IntPred k)=k"))
(simp "<-" (pf "IntS(IntPred k0)=k0"))
(use "IntLtMonIntS")
(use "IntPred k<IntPred k0")
(use "Truth")
(use "Truth")
;; 4
(use "IntLtMonIntPred")
;; Proof finished.
;; (cp)
(add-rewrite-rule "IntPred k<IntPred k0" "k<k0")
...
(assume "k<=k0")
(use "IntNotLtToLe")
(assume "IntS k0<IntS k")
(assert "k<k")
(use "IntLeLtTrans" (pt "k0"))
(use "k<=k0")
(use "IntS k0<IntS k")
(assume "Absurd")
(use "Absurd")
...
(use "IntLeLtTrans" (pt "IntPred k0"))
(use "IntPred k<=IntPred k0")
(use "k0<k")
(assume "Absurd")
(use "Absurd")
...
(assume "k<=k0")
(use "IntNotLtToLe")
(assume "IntPred k0<IntPred k")
(assert "k<k")
(use "IntLeLtTrans" (pt "k0"))
(use "k<=k0")
(use "IntPred k0<IntPred k")
(assume "Absurd")
(use "Absurd")
;; Proof finished.
;; (cp)
...
(set-goal "all k,k0,p (k*p<=k0*p)=(k<=k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; Proof finished
;; (cp)
(add-rewrite-rule "k*p<=k0*p" "k<=k0")

;; IntLeTimesIntPCancelL
(set-goal "all p,k,k0 (p*k<=p*k0)=(k<=k0)")
(assume "p" "k" "k0")
(simp "IntTimesComm")
(simp (pf "p*k0=k0*p"))
(use "Truth")
...
;; Proof finished
;; (cp)
(add-rewrite-rule "p*k<=p*k0" "k<=k0")
...
(assert "k0*p<=k*p")
 (use "k0<=k")
(use-with "IntLtLeTrans" (pt "k*p") (pt "k0*p") (pt "k*p") "kp<k0p")
;; 4
(assume "k<k0")
(use "IntNotLeToLt")
(assume "k0p<=kp")
(assert "k0<=k")
 (use "k0p<=kp")
(use-with "IntLtLeTrans" (pt "k") (pt "k0") (pt "k") "k<k0")
;; Proof finished.
;; (cp)
(add-rewrite-rule "k*p<k0*p" "k<k0")
...
(set-goal "all p,k,k0 (IntN p*k<=IntN p*k0)=(k0<=k)")
(assume "p" "k" "k0")
(simp "IntTimesIntNL")
(simp "IntTimesIntNL")
(use "Truth")
;; Proof finished.
;; (cp)
(add-rewrite-rule "IntN p*k<=IntN p*k0" "k0<=k")

;; IntLeTimesIntNCancelR
(set-goal "all p,k,k0 (k*IntN p<=k0*IntN p)=(k0<=k)")
(assume "p" "k" "k0")
(simp "IntTimesIntNR")
(simp "IntTimesIntNR")
(use "Truth")
;; Proof finished.
;;(cp)
(add-rewrite-rule "k*IntN p<=k0*IntN p" "k0<=k")
...
(use "IntNotLeToLt")
(assume "k<=k0")
(assert "IntN p*k0<=IntN p*k")
 (use "k<=k0")
(assume "-pk0<=~pk")
(use-with "IntLtLeTrans" (pt "IntN p*k") (pt "IntN p*k0") (pt "IntN p*k")
	  "-pk<-k0p" "-pk0<=~pk")
;; 4
(assume "k0<k")
(use "IntNotLeToLt")
...
(assert "k<=k0")
 (use "-pk0<=-pk")
(use-with "IntLtLeTrans" (pt "k0") (pt "k") (pt "k0") "k0<k")
;; Proof finished.
;; (cp)
(add-rewrite-rule "IntN p*k<IntN p*k0" "k0<k")
...
;; IntLeTimesCancelL
(set-goal "all k,k0,k1(IntZero<k -> k*k0<=k*k1 -> k0<=k1)")
(assert "all k0,k1,k(IntZero<k -> k*k0<=k*k1 -> k0<=k1)")
(assume "k0" "k1")
(cases)
;; 5-7
(assume "p" "Useless" "pk0<=pk1")
(use "pk0<=pk1")
;; 6
(assume "Absurd" "Useless")
(use "EfAtom")
...
;; IntLeTimesCancelR
(set-goal "all k,k0,k1(IntZero<k0 -> k*k0<=k1*k0 -> k<=k1)")
(assume "k" "k0" "k1")
(simp "IntTimesComm")
(simp (pf "k1*k0=k0*k1"))
...
;; IntLeLtCases
(set-goal "all k,k0((k<=k0 -> Pvar) -> (k0<k -> Pvar) -> Pvar)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLeCases
(set-goal "all k,k0(k<=k0 -> (k<k0 -> Pvar) -> (k=k0 -> Pvar) -> Pvar)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLeMonTimes
(set-goal "all k,k0,k1(0<=k -> k0<=k1 -> k0*k<=k1*k)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; For IntLeMonPlus : k<=k0 -> k1<=k2 -> k+k1<=k0+k2
;; it suffices to prove k<=k0 -> k+k1<=k0+k1
...
;; IntLeMonPlusAux k0<=k1 -> k+k0<=k+k1 uses
;;   IntLePlusIntN IntLeMonPlusIntN IntLeMonPlusIntP IntLePlusIntP

;; IntLeMonPlus k<=k0 -> k1<=k2 -> k+k1<=k0+k2
...
(assume "k" "k0" "k1" "k2" "k<=k0" "k1<=k2")
(use "IntLeTrans" (pt "k+k2"))
(use "IntLeMonPlusAux")
(use "k1<=k2")
(simp "IntPlusComm")
(use "IntLeTrans" (pt "k2+k0"))
(use "IntLeMonPlusAux")
(use "k<=k0")
(simp "IntPlusComm")
(use "Truth")
...
;; IntLtIntS
(set-goal "all k,k0 (k<IntS k0)=(k<=k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLeIntS
(set-goal "all k,k0(IntS k<=k0)=(k<k0)")
(assume "k" "k0")
(inst-with-to "IntLtIntS" (pt "IntS k") (pt "k0") "IntLtIntSInst")
(ng "IntLtIntSInst")
(simp "IntLtIntSInst")
(use "Truth")
;; Proof finished.
...
;; (cp)
(save "IntLeAbsPlus")
(add-rewrite-rule "abs(k+k0)<=abs k+abs k0" "True")

;; IntLeMinusAbs
(set-goal "all k,k0(abs k+ ~abs k0<=abs(k+ ~k0))")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; IntLeAbs
(set-goal "all k,k0(k<=k0 -> ~k<=k0 -> abs k<=k0)")
(cases)
(cases)
(assume "k0" "1<=k0" "Useless")
(use "1<=k0")
(assume "p" "k0" "2p<=k0" "Useless")
(ng)
(use "2p<=k0")
(assume "p" "k0" "2p+1<=k0" "Useless")
(ng)
(use "2p+1<=k0")
(assume "k0" "0<=k0" "Useless")
(use "0<=k0")
(assume "p" "k0" "Useless" "p<=k0")
(ng)
(use "p<=k0")
;; Proof finished.
;; (cp)
...
;; IntLeMonMinus
(set-goal "all k,k0,k1,k2(k<=k0 -> k2<=k1 -> k-k1<=k0-k2)")
(assume "k" "k0" "k1" "k2" "k<=k0" "k2<=k1")
(ng)
(use "IntLeMonPlus")
(use "k<=k0")
(simp "IntLeUMinus")
...
;; (cp)
(save "IntMinusPlusEq")
(add-rewrite-rule "k-k0+k0" "k")

;; IntLeAbsMinus
(set-goal "all k,k1,k0 abs(k+ ~k1)<=abs(k+ ~k0)+abs(k0+ ~k1)")
(assume "k" "k1" "k0")
(assert "k+ ~k1=(k+ ~k0)+(k0+ ~k1)")
 (simp "<-" "IntPlusAssoc")
 (ng)
 (use "Truth")
(assume "Assertion")
...
(save "IntLeAbsMinus")
;; (cp)
(add-rewrite-rule "abs(k+ ~k1)<=abs(k+ ~k0)+abs(k0+ ~k1)" "True")
...
(assume "k" "k0" "k1" "k2" "k<k0" "k1<=k2")
(simp "<-" "IntLeIntS")
(use-with "IntLeMonPlus" (pt "IntS k") (pt "k0") (pt "k1") (pt "k2") "?" "?")
(simp "IntLeIntS")
(use "k<k0")
(use "k1<=k2")
;; Proof finished.
...
(assume "k" "k0" "k1" "k2" "k<=k0" "k1<k2")
(simp "<-" "IntLeIntS")
(use-with "IntLeMonPlus" (pt "k") (pt "k0") (pt "IntS k1") (pt "k2") "?" "?")
(use "k<=k0")
(simp "IntLeIntS")
(use "k1<k2")
;; Proof finished.
...
;; IntPlusCancelR
(set-goal "all k,k0,k1(k+k0=k1+k0 -> k=k1)")
(assert "all k0,k,k1(k+k0=k1+k0 -> k=k1)")
(cases)
;; 4-6
(ind)
(assume "k" "k1")
...
;; IntPlusCancelL
(set-goal "all k,k0,k1(k+k0=k+k1 -> k0=k1)")
(assume "k" "k0" "k1")
(simp "IntPlusComm")
(simp (pf "k+k1=k1+k"))
(use "IntPlusCancelR")
...
;; BooleEqTrans
(set-goal
 "all boole,boole0,boole1(boole=boole0 -> boole0=boole1 -> boole=boole1)")
(assume "boole" "boole0" "boole1" "=Hyp")
(simp "<-" "=Hyp")
(assume "boole=boole1")
(use "boole=boole1")
;; Proof finished.
...
(simp "IntPlusIdIntPSZero")
(simp "IntPlusIdIntPSZero")
(use "BooleEqTrans" (pt "k+p+p<=k1+p+p"))
(use "Truth")
(use "BooleEqTrans" (pt "k+p<=k1+p"))
(use "IH")
(use "IH")
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "k+k0<=k1+k0" "k<=k1")
...
;; Proof finished.
;; (cp)
(add-rewrite-rule "k+k0<=k+k1" "k0<=k1")

;; IntLtPlusCancelR
(set-goal "all k,k0,k1 (k+k0<k1+k0)=(k<k1)") ;as rewrite rule
(assume "k" "k0" "k1")
(use "BooleAeqToEq")
;; 3,4
(assume "k+k0<k1+k0")
...
(ng)
(assume "k1<=k")
(use-with "IntLtLeTrans" (pt "k") (pt "k1") (pt "k") "k<k1" "k1<=k")
;; Proof finished.
;; (cp)
(add-rewrite-rule "k+k0<k1+k0" "k<k1")

;; IntLtPlusCancelL
(set-goal "all k,k0,k1 (k+k0<k+k1)=(k0<k1)") ;as rewrite rule
(assume "k" "k0" "k1")
(simp "IntPlusComm")
(simp (pf "k+k1=k1+k"))
(use "Truth")
...
;; IntLePlusL
(set-goal "all k,k0,k1(k0<= ~k+k1 -> k+k0<=k1)")
(assume "k" "k0" "k1" "k0<= ~k+k1")
...
;; IntLeAbsMinus1
(set-goal "all k,k0,k1(abs(k+ ~k0)<=k1 -> k<=k0+k1)")
(assume "k" "k0" "k1" "LeHyp")
(use "IntLePlusR")
(simp "IntPlusComm")
(use "IntLeTrans" (pt "abs(k+ ~k0)"))
(use "Truth")
(use "LeHyp")
;; Proof finished.
;; (cp)
...
;; IntAbsPlusUMinus
(set-goal "all k,k0 abs(k+ ~k0)=abs(k0+ ~k)")
(assume "k" "k0")
(simp "<-" "IntAbs0RewRule")
(ng #t)
(simp "IntPlusComm")
...
;; IntLeAbsMinus2
(set-goal "all k,k0,k1(abs(k+ ~k0)<=k1 -> k0<=k+k1)")
(assume "k" "k0" "k1" "LeHyp")
(use "IntLePlusR")
(use "IntLeTrans" (pt "abs(~k+k0)"))
(use "Truth")
...
;; IntMaxLUB
(set-goal "all k,k0,k1(k<=k1 -> k0<=k1 -> k max k0<=k1)")
(cases)
;; 2-4
(assume "p")
(cases)
...
(use "Absurd")
;; 7
(assume "k1" "p<=k1" "Useless")
(use "p<=k1")
;; 8
(assume "q" "k1" "p<=k1" "Useless")
(use "p<=k1")
;; 3
(cases)
;; 19-21
(assume "p" "k1" "Useless" "p<=k1")
(use "p<=k1")
;; 20
(assume "k1" "0<=k1" "Useless")
(use "0<=k1")
;; 21
(assume "q" "k1" "0<=k1" "Useless")
(use "0<=k1")
;; 4
(assume "p")
(cases)
;; 26-28
(assume "q" "k1" "Useless" "q<=k1")
(use "q<=k1")
;; 27
(assume "k1" "Useless" "0<=k1")
(use "0<=k1")
;; 28
(assume "q")
(cases)
...
;; IntLeMonMax
(set-goal "all k,k0,k1,k2(k<=k0 -> k1<=k2 -> k max k1<=k0 max k2)")
(assert "all k,k0,k1(k<=k0 -> k max k1<=k0 max k1)")
 (assume "k" "k0" "k1" "k<=k0")
 (use "IntMaxLUB")
 (use "IntLeTrans" (pt "k0"))
 (use "k<=k0")
 (use "IntMaxUB1")
 (use "IntMaxUB2")
;; Assertion proved
(assume "Assertion1")
(assert "all k,k0,k1(k0<=k1 -> k max k0<=k max k1)")
 (assume "k" "k0" "k1" "k0<=k1")
 (use "IntMaxLUB")
 (use "IntMaxUB1")
 ...
 (use "IntMaxUB2")
;; Assertion proved
(assume "Assertion2" "k" "k0" "k1" "k2" "k<=k0" "k1<=k2")
(use "IntLeTrans" (pt "k0 max k1"))
(use "Assertion1")
(use "k<=k0")
(use "Assertion2")
(use "k1<=k2")
...
;; IntMaxComm
(set-goal "all k,k0 k max k0=k0 max k")
(assume "k" "k0")
(use "IntLeAntiSym")
(use "IntMaxLUB")
(use "IntMaxUB2")
...
;; IntMaxAssoc
(set-goal "all k,k0,k1 k max(k0 max k1)=k max k0 max k1")
(assume "k" "k0" "k1")
(use "IntLeAntiSym")
;; 3,4
(use "IntMaxLUB")
(use "IntLeTrans" (pt "k max k0"))
(use "IntMaxUB1")
(use "IntMaxUB1")
(use "IntLeMonMax")
(use "IntMaxUB2")
...
;; (cp)
(save "IntMaxAssoc")
(add-rewrite-rule "k max(k0 max k1)" "k max k0 max k1")

;; IntMaxEq1
(set-goal "all k,k0(k0<=k -> k max k0=k)")
(assume "k" "k0" "k0<=k")
(use "IntLeAntiSym")
(use "IntMaxLUB")
(use "Truth")
...
;; IntMaxEq2
(set-goal "all k,k0(k<=k0 -> k max k0=k0)")
(assume "k" "k0" "k<=k0")
(use "IntLeAntiSym")
(use "IntMaxLUB")
(use "k<=k0")
...
;; IntMaxEq1Rev
(set-goal "all k,k0(k max k0=k -> k0<=k)")
(assume "k" "k0" "EqH")
(simp "<-" "EqH")
(use "IntMaxUB2")
;; Proof finished.
...
;; IntMaxEq2Rev
(set-goal "all k,k0(k max k0=k0 -> k<=k0)")
(assume "k" "k0" "EqH")
(simp "<-" "EqH")
(use "IntMaxUB1")
;; Proof finished.
...
;; IntMinGLB
(set-goal "all k,k0,k1(k1<=k -> k1<=k0 -> k1<=k min k0)")
(cases)
;; 2-4
(assume "p")
(cases)
...
;; 7
(ng)
(assume "k1" "Useless" "k1<=0")
(use "k1<=0")
;; 8
(ng)
(assume "q" "k1" "Useless" "k1<=Nq")
(use "k1<=Nq")
;; 3
(cases)
;; 22-24
(ng)
(assume "p" "k1" "k1<=0" "Useless")
(use "k1<=0")
;; 23
(ng)
(assume "k1" "Useless" "k1<=0")
...
;; 32-34
(ng)
(assume "q" "k1" "k1<=Np" "Useless")
(use "k1<=Np")
;; 33
(ng)
(assume "k1" "k1<=Np" "Useless")
(use "k1<=Np")
;; 34
(assume "q")
(cases)
...
;; IntLeMonMin
(set-goal "all k,k0,k1,k2(k<=k0 -> k1<=k2 -> k min k1<=k0 min k2)")
(assert "all k,k0,k1(k<=k0 -> k min k1<=k0 min k1)")
 (assume "k" "k0" "k1" "k<=k0")
 (use "IntMinGLB")
(use "IntLeTrans" (pt "k"))
(use "IntMinLB1")
...
;; Assertion proved
(assume "Assertion1")
(assert "all k,k0,k1(k0<=k1 -> k min k0<=k min k1)")
(assume "k" "k0" "k1" "k0<=k1")
(use "IntMinGLB")
(use "IntMinLB1")
(use "IntLeTrans" (pt "k0"))
...
;; IntMinComm
(set-goal "all k,k0 k min k0=k0 min k")
(assume "k" "k0")
(use "IntLeAntiSym")
(use "IntMinGLB")
(use "IntMinLB2")
...
;; IntMinAssoc
(set-goal "all k,k0,k1 k min(k0 min k1)=k min k0 min k1")
(assume "k" "k0" "k1")
(use "IntLeAntiSym")
;; 3,4
(use "IntMinGLB")
...
;; (cp)
(save "IntMinAssoc")
(add-rewrite-rule "k min(k0 min k1)" "k min k0 min k1")

;; IntMinEq1
(set-goal "all k,k0(k<=k0 -> k min k0=k)")
(assume "k" "k0" "k<=k0")
(use "IntLeAntiSym")
(use "IntMinLB1")
(use "IntMinGLB")
...
;; IntMinEq2
(set-goal "all k,k0(k0<=k -> k min k0=k0)")
(assume "k" "k0" "k0<=k")
(use "IntLeAntiSym")
(use "IntMinLB2")
(use "IntMinGLB")
...
;; IntMinEq1Rev
(set-goal "all k,k0(k min k0=k -> k<=k0)")
(assume "k" "k0" "EqH")
(simp "<-" "EqH")
(use "IntMinLB2")
;; Proof finished.
...
;; IntMinEq2Rev
(set-goal "all k,k0(k min k0=k0 -> k0<=k)")
(assume "k" "k0" "EqH")
(simp "<-" "EqH")
(use "IntMinLB1")
;; Proof finished.
...
;; IntTimesMaxDistr
(set-goal "all k,k0,k1(0<=k -> k*(k0 max k1)=k*k0 max(k*k1))")
(cases)
;; 2-4
(assert "all p,k0,k1 p*(k0 max k1)=p*k0 max(p*k1)")
(ind)
;; 7-9
(assume "k0" "k1")
(use "Truth")
;; 8
(assume "p" "IH" "k0" "k1")
(use "IntLeLtCases" (pt "k0") (pt "k1"))
;; 12,13
(assume "k0<=k1")
(simp (pf "k0 max k1=k1"))
(simp (pf "SZero p*k0 max(SZero p*k1)=SZero p*k1"))
(use "Truth")
(use "IntMaxEq2")
(use "k0<=k1")
(use "IntMaxEq2")
...
;; 13
(assume "k1<k0")
(inst-with-to "IntLtToLe" (pt "k1") (pt "k0") "k1<k0" "k1<=k0")
(simp (pf "k0 max k1=k0"))
(simp (pf "SZero p*k0 max(SZero p*k1)=SZero p*k0"))
(use "Truth")
(use "IntMaxEq1")
(use "k1<=k0")
(use "IntMaxEq1")
...
(assume "k0<=k1")
(simp (pf "k0 max k1=k1"))
(simp (pf "SOne p*k0 max(SOne p*k1)=SOne p*k1"))
(use "Truth")
(use "IntMaxEq2")
(use "k0<=k1")
(use "IntMaxEq2")
...
(inst-with-to "IntLtToLe" (pt "k1") (pt "k0") "k1<k0" "k1<=k0")
(simp (pf "k0 max k1=k0"))
(simp (pf "SOne p*k0 max(SOne p*k1)=SOne p*k0"))
(use "Truth")
(use "IntMaxEq1")
(use "k1<=k0")
(use "IntMaxEq1")
(use "k1<=k0")
;; Assertion proved.
(assume "Assertion" "p" "k0" "k1" "Useless")
(use "Assertion")
;; 3
(assume "k0" "k1" "Useless")
(use "Truth")
;; 4
(assume "p" "k0" "k1" "Absurd")
(use "EfAtom")
(use "Absurd")
;; Proof finished.
;; (cp)
...
(add-computation-rules
 ...
 "PosQR(SZero p)q"
 "[if (PosQR p q)
      ([k,k0] [if (2*k0<q) (2*k pair 2*k0) (2*k+1 pair 2*k0-q)])]"

 "PosQR(SOne p)q"
 "[if (PosQR p q)
      ([k,k0] [if (2*k0+1<q) (2*k pair 2*k0+1) (2*k+1 pair 2*k0+1-q)])]")
...
;; PosQRCorrAux
(set-goal
 "all p,q,k,k0(PosQR p q=(k pair k0) -> p=k*q+k0 andnc 0<=k0 andnc k0<q)")
...
(assume "2*rht(PosQR p q)=k0")
(simp "<-" "2*rht(PosQR p q)=k0")
(split)
(use "IHAux")
(use "InstIH")
;; 93
...
(assume "2*rht(PosQR p q)+IntN q=k0")
(simp "<-" "2*rht(PosQR p q)+IntN q=k0")
(split)
(ng #t)
(simp "<-" "IntPlusAssoc")
(simp "<-" "IntPlusAssoc")
...
;; (pp "IntLtMonPlus1")
;; all k,k0,i,k2(k<k0 -> i<=k2 -> k+i<k0+k2)
...
(assume "2*lft(PosQR p q)=k")
(ng #t)
(assume "IntS(2*rht(PosQR p q))=k0")
(simp "<-" "IntS(2*rht(PosQR p q))=k0")
(split)
(ng #t)
;;   IHAux:all p,q(
;;          p=lft(PosQR p q)*q+rht(PosQR p q) -> 
;;          SZero p=2*lft(PosQR p q)*q+2*rht(PosQR p q))
;;   p16822  p  IH:all q,k,k0(PosQR p q=(k pair k0) -> p=k*q+k0 andnc 0<=k0 andnc k0<q)
;;   q  k  k0  Assertion:all kj kj=(lft kj pair rht kj)
...
(assume "2*lft(PosQR p q)+1=k")
(ng #t)
(assume "IntS(2*rht(PosQR p q)+IntN q)=k0")
(simp "<-" "IntS(2*rht(PosQR p q)+IntN q)=k0")
(split)
(simp "<-" "2*lft(PosQR p q)+1=k")
(ng #t)
(simp "<-" "IntPlusAssoc")
...
;; PairConstrOneTwoInt
(set-goal "all kj (lft kj pair rht kj)eqd kj")
(cases)
(strip)
(use "InitEqD")
;; Proof finished.
;; (cp)
(save "PairConstrOneTwoInt")
...
;; Code discarded 2025-10-03.  PairConstrOneTwo needs totally inhabited types
;; ;; PosQRCorr
;; (set-goal "all p,q(
;;      p=lft(PosQR p q)*q+rht(PosQR p q) andnc 
;;      0<=rht(PosQR p q) andnc rht(PosQR p q)<q)")
...
;; IntPosTimes
(set-goal "all k,k0(0<k*k0 -> 0<=k -> 0<k0)")
(cases)
(assume "p")
(cases)
(strip)
...
;; IntExpTimes
(set-goal "all k,k0,n (k*k0)**n=k**n*k0**n")
(assume "k" "k0")
(ind)
(use "Truth")
(assume "n" "IH")
