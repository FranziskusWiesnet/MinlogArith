...
;; (pp "CoEqPStrClause")
;; all xstr^,xstr^0(
;;  CoEqPStr xstr^ xstr^0 -> 
;;  exr x^,x^0(
;;   EqP x^ x^0 andd 
;;   exr xstr^1,xstr^2(
;;    CoEqPStr xstr^1 xstr^2 andl 
;;    xstr^ eqd(x^ :~:xstr^1) andnc xstr^0 eqd(x^0:~:xstr^2))))
(add-co "EqPStrNc")

;; An inverse to CoTotalStrClause
;; CoTotalStrSCons
(set-goal "all x^(Total x^ ->
 all xstr^(CoTotalStr xstr^ -> CoTotalStr(x^ :~:xstr^)))")
(assume "x^" "Tx")
...
(set-goal "all xstr^(CoTotalStr xstr^ -> CoSTotalStr xstr^)")
(assume "xstr^" "CoTxstr")
(coind "CoTxstr")
...
(set-goal "all xl^(TotalList xl^ ->
 all xstr^(CoTotalStr xstr^ -> CoTotalStr(xl^ ++xstr^)))")
(assume "xl^" "Txl")
(elim "Txl")
(ng)
(auto)
(assume "x^" "Tx" "xl^1" "Txl1" "IH" "xstr^" "CoTxstr")
(ng #t)
(use "CoTotalStrSCons")
(use "Tx")
(use "IH")
...
;; [xl]
;;  (Rec list alpha=>str alpha=>str alpha)xl([xstr]xstr)
;;  ([x,xl0,(str alpha=>str alpha),xstr]
;;    (cCoTotalStrSCons alpha)x((str alpha=>str alpha)xstr))

;; CoSTotalStrAppd
(set-goal "all xl^(STotalList xl^ ->
 all xstr^(CoSTotalStr xstr^ -> CoSTotalStr(xl^ ++xstr^)))")
(assume "xl^" "STxl")
(elim "STxl")
(ng)
(auto)
(assume "x^" "xl^1" "STxl1" "IH" "xstr^" "CoSTxstr")
(ng #t)
...
(add-program-constant "StrMap" (py "(alpha=>beta)=>str alpha=>str beta"))

(add-infix-display-string "StrMap" "smap" 'pair-op) ;right associative

(add-var-name "phi" (py "alpha=>beta"))
(add-var-name "y" (py "alpha"))
(add-var-name "ystr" (py "str alpha"))
...
(add-var-name "z" (py "beta"))
(add-var-name "zstr" (py "str beta"))

;; StrMapCoTotal
(set-goal "all phi,ystr^(CoTotalStr ystr^ -> CoTotalStr(phi smap ystr^))")
(use "AllTotalIntro")
(assume "phi^" "Tphi")
;; We prove an auxiliary proposition, by coinduction.
(assert "allnc zstr^(
...
;; [phi,xstr]
;;  (CoRec str alpha=>str beta)xstr
;;  ([xstr0][case (DesYprod xstr0) (x pair xstr1 -> phi x pair InR xstr1)])
...
(inst-with-to "CoSTotalStrClause" (py "alpha") (pt "ystr^") "CoSTystr" "Inst")
...
(set-goal "all xstr^(CoTotalStr xstr^ -> Total(Head xstr^))")
(assume "xstr^" "CoTxstr")
(inst-with-to "CoTotalStrClause"
...
;; CoTotalStrTail
(set-goal "all xstr^(CoTotalStr xstr^ -> CoTotalStr(Tail xstr^))")
(assume "xstr^" "CoTxstr")
(inst-with-to "CoTotalStrClause"
	      (pt "xstr^") "CoTxstr" "CoTotalStrClauseInst" )
(by-assume "CoTotalStrClauseInst" "x^1" "x1Prop")
...
;; CoSTotalStrTail
(set-goal "all xstr^(CoSTotalStr xstr^ -> CoSTotalStr(Tail xstr^))")
...
(set-goal "all n,xstr^(CoTotalStr xstr^ -> Total(n init xstr^))")
(ind)
(strip)
(use "TotalListNil")
...
(set-goal "all n,xstr^(CoSTotalStr xstr^ -> STotalList(n init xstr^))")
(ind)
(strip)
(use "STotalListNil")
...
;; StrInitAppd
(set-goal
 "allnc xl^,xstr^(STotalList xl^ -> Lh xl^ init(xl^ ++xstr^)eqd xl^)")
(assume "xl^" "xstr^" "STxl")
(elim "STxl")
(ng)
(use "InitEqD")
...
(set-goal "all n,xl^(STotalList xl^ ->
 all xstr^ (n+Lh xl^)init(xl^ ++xstr^)eqd xl^ ++(n init xstr^))")
(assume "n" "xl^" "STxl")
(elim "STxl")
(ng)
(strip)
...
;; StrInitPlusAppd
(set-goal "all n,xl,xstr^ (n+Lh xl)init(xl++xstr^)eqd xl++(n init xstr^)")
(assume "n" "xl" "xstr^")
...
;; CoTotalStrRest
(set-goal "all n,xstr^(CoTotalStr xstr^ -> CoTotalStr(n rest xstr^))")
(ind)
(assume "xstr^" "CoTxstr")
(use "CoTxstr")
(assume "n" "IH" "xstr^" "CoTxstr")
...
;; StrAppdInitRestGen
(set-goal "all xl,n,xstr^((Lh xl+n)init(xl++xstr^)eqd xl ++(n init xstr^))")
(ind)
(ng)
(strip)
(use "InitEqD")
;; 3
(assume "x" "xl" "IH" "n" "xstr^")
(ng)
(simp "IH")
(use "InitEqD")
;; Proof finished.
...
(set-goal "all n,xstr^(CoTotalStr xstr^ -> Total(n node xstr^))")
(ind)
(strip)
(use "TotalListNil")
(assume "n" "IH" "xstr^" "CoTxstr")
...
(set-goal "all n,xstr^(CoSTotalStr xstr^ -> STotalList(n node xstr^))")
(ind)
(strip)
(use "STotalListNil")
(assume "n" "IH" "xstr^" "CoSTxstr")
...
;; StrNodeAppd
(set-goal
 "all xl^,xstr^(STotalList xl^ -> Lh xl^ node(xl^ ++xstr^)eqd Rev xl^)")
(assume "xl^" "xstr^" "STxl")
(elim "STxl")
(ng)
(use "InitEqD")
...
(set-goal "all n,xl^(STotalList xl^ -> allnc xstr^(CoSTotalStr xstr^ -> 
 (n+Lh xl^)node(Rev xl^ ++xstr^)eqd n node xstr^ ++xl^))")
(assume "n" "xl^" "STxl" "xstr^" "CoSTxstr")
;; Revert both sides of StrInitPlusAppdPartial at Rev xl
(assert
 "Rev((n+Lh Rev xl^)init(Rev xl^ ++xstr^))eqd Rev(Rev xl^ ++(n init xstr^))")
...
;; StrNodePlusAppd
(set-goal "all n,xl,xstr^(CoSTotalStr xstr^ -> 
 (n+Lh xl)node(Rev xl ++xstr^)eqd n node xstr^ ++xl)")
(assume "n" "xl" "xstr^")
...
;; StrRestAppd
(set-goal
 "all xl^,xstr^(STotalList xl^ -> Lh xl^ rest(xl^ ++xstr^)eqd xstr^)")
(assume "xl^" "xstr^" "STxl")
(elim "STxl")
(ng)
(use "InitEqD")
...
;; StrAppdAssoc
(set-goal "all xl^1,xl^2,xstr^(STotalList xl^1 ->
 xl^1++(xl^2++xstr^)eqd xl^1++xl^2++xstr^)")
(assume "xl^1" "xl^2" "xstr^" "STxl1")
(elim "STxl1")
(ng)
(use "InitEqD")
(assume "x^" "xl^" "STxl" "EqHyp")
(ng)
(simp "EqHyp")
(use "InitEqD")
;; Proof finished.
...
;; StrNodeSum
(set-goal "all xl^1,xl^2,xstr^(
     STotalList xl^1 -> 
     STotalList xl^2 -> 
     Lh(xl^1++xl^2)node(xl^1++xl^2++xstr^)eqd 
     Lh xl^2 node(Lh xl^1 rest(xl^1++xl^2++xstr^))++
     (Lh xl^1 node(xl^1++xl^2++xstr^)))")
(assume "xl^1" "xl^2" "xstr^" "STxl1" "STxl2")
(simp "StrNodeAppd")
(simp "<-" "StrAppdAssoc")
(simp "StrNodeAppd")
(simp "StrRestAppd")
...
(add-computation-rules
 ...
 "Succ n snr xstr"
 "[if (n snr xstr) ([xl,xstr1](Head xstr1::xl)pair Tail xstr1)]")
...
;; 2025-10-19.  Material on Path commented out.  It is not needed, and
;; some of its properties need a global assumption StrPathAxiom
...
(set-goal "all f^(
 all x^(
  ...
  all x^((Pvar alpha)x^ -> CoSTotalStr((CoRec alpha=>str beta)x^ f^)))")
(assume "f^" "fHyp")
;; We prove an auxiliary proposition, by coinduction.
(assert "all ystr^(
 ...
 all x^(
  ...
  exr y^,s^(
   ...
   (exr ystr^(CoTotalStr ystr^ andl s^ eqd(InL (str beta) alpha)ystr^) ord 
    exr x^1((Pvar alpha)x^1 andl s^ eqd(InR alpha (str beta))x^1)))) -> 
  all x^((Pvar alpha)x^ -> CoTotalStr((CoRec alpha=>str beta)x^ f^)))")
(assume "f^" "fHyp")
;; We prove an auxiliary proposition, by coinduction.
(assert "all ystr^(
  exr x^((Pvar alpha)x^ andl ystr^ eqd(CoRec alpha=>str beta)x^ f^) -> 
...
;;      (z pair(str beta ysum gamma) -> 
;;      [case (str beta ysum gamma)
;;        (InL zstr -> z pair InL zstr)
;;        (InR gamma_1 -> z pair InR gamma_1)])])

;; (display-default-varnames)

(remove-var-name
 "f" "s" "zstr" "z" "phi" "xxstr" "xstr" "xl" "x" "y")
