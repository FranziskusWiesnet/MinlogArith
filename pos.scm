...
;; NatLePosToNatToPos added.  Franziskus Wiesnet's pos_add.scm
;; included.  Theorems NatLePosExpNatF PosEqSym PosLeToNotLt
;; PosLtMonTimes PosLtPosToLtTimes PosLeTimesPos PosLeNotEqToLt
;; PosExpTwoSucc PosThirdBinom added.  pconst PosSqare added.
;; Theorems PosLtMonSquare PosLeMonSquare PosLeMonSquareInv added.
;; pconst ExBPos added, with if instead of orb (for efficiency).
;; pconst PosLeast added.
...
;; NatLePosToNatToPos
(set-goal "all n,m(n<=m -> n<=PosToNat(NatToPos m))")
(assume "n")
(cases)
;; Cases 1
(assume "n<=0")
(use "NatLeTrans" (pt "Zero"))
;; NatLeTrans 1
(use "n<=0")
;; NatLeTrans 2
(use "Truth")
...
;; ExBPosElim
(set-goal "all p,wf(ExBPos p wf -> allnc q(q<=p -> wf q -> Pvar) -> Pvar)")
(ind)
(strip)
(ng)
(use 2 (pt "1"))
